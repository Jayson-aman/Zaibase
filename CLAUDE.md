...
| **一問一答の解説が、式をなぞるだけで「なぜそうなるか」を言っていない**（1,770問中1,221問＝69%）。「頂角40度の二等辺三角形の底角は」に「底角は2つとも等しいので (180−40)÷2＝70度」とだけ書いてあり、**なぜ底角が等しいのか**も、**140度が底角2つぶんの合計だ**という肝心なところも書いていなかった。ユーザーから「説明不足で、わからない人はわからない」と指摘された | 監査の「一問一答の解説が式だけで理由が書いていない（減らしていく数字）」。これは壊れているわけではないので0件にはならない。**増えたら書き方が後戻りしている**。書くときの型：①公式が何を表しているか（面積なら「1cmの正方形が何個か」）②なぜその操作をするのか ③計算 ④よくあるまちがい。数だけ書いて終わりにしない |
