...
export const lessonFigsKokoMath: Record<string, Figure> = {
  ...
  lf_kmath_13: {
    ...
    // 図が組み立てられていく順に合わせた段階説明。
    // 円錐の描画は 底面 → 母線 → 母線の長さ → 半径 → 高さ → 直角 → 高さの長さ
    // の順に部品が出るので、その流れと同じ順序にしてある。
    steps: [
      '円錐を、頂点を通るようにまっすぐ切ったところを考える。切り口の中に直角三角形が現れる。',
      '母線（頂点から底面のふちまで）が、その直角三角形の斜辺になる。ここでは10cm。',
      '底面の半径（中心からふちまで）が、直角をはさむ一方の辺になる。ここでは6cm。',
      '頂点から底面の中心へまっすぐ下ろした線が高さ h で、これがもう一方の辺。中心のところが直角になる。',
      '三平方の定理 h² ＋ 6² ＝ 10² にあてはめる。h² ＝ 100 − 36 ＝ 64。',
      'h ＝ √64 ＝ 8cm。この三角形は 6 : 8 : 10 で、3 : 4 : 5 をちょうど2倍したものになっている。',
    ],
    // 4枚目（高さと直角）で図は描き上がる。5・6枚目は完成した図のまま計算する。
    buildSteps: 4,
