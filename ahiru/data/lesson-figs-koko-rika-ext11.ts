...
export const lessonFigsKokoRikaExt11: Record<string, Figure> = {
  ...
  lf_kokorikaext11_384: {
    ...
    steps: [
      "よこじくは「マグマの種類」、たてじくは「二酸化ケイ素の割合（％）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「流紋岩質」で70（二酸化ケイ素の割合（％））。",
      "いちばん小さいのは「玄武岩質」で50。差は20。",
      "大きい順にならべると 流紋岩質 → 安山岩質 → 玄武岩質。棒の高さを見比べて確かめる。",
      "マグマにふくまれる二酸化ケイ素（SiO₂）のおおよその割合。この値が大きいほどマグマのねばりけは強くなり、色は白っぽく、火山の形は急になり、噴火は爆発的になる。左から順に「ゆるやかな火山→円すい形→ドーム状」と対応する。50％・60％・70％はおおよその目安の値",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokorikaext11_388: {
    ...
    steps: [
      ...
      "いちばん大きいのは「長石（無色鉱物）」で、全体のおよそ50%。",
      "内わけは 長石（無色鉱物） 50%、石英（無色鉱物） 30%、黒雲母（有色鉱物） 15%、その他の有色鉱物 5%。",
  ...
  lf_kokorikaext11_389: {
    ...
    steps: [
      "よこじくは「火成岩の種類（上：火山岩／下：深成岩）」、たてじくは「有色鉱物の割合（％）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「玄武岩・はんれい岩」で60（有色鉱物の割合（％））。",
      "いちばん小さいのは「流紋岩・花こう岩」で10。差は50。",
      "大きい順にならべると 玄武岩・はんれい岩 → 安山岩・せん緑岩 → 流紋岩・花こう岩。棒の高さを見比べて確かめる。",
      "火成岩にふくまれる有色鉱物（黒雲母・角閃石・輝石・カンラン石）のおおよその割合。右へ行くほど有色鉱物が増え、岩石は黒っぽくなる。残りはすべて無色鉱物（石英・長石）なので、無色鉱物の割合は左から90％・65％・40％となる。二酸化ケイ素の割合（70％・60％・50％）とは逆向きに変化することに注意",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokorikaext11_391: {
    ...
    steps: [
      "よこじくは「地震発生からの時間（秒）」、たてじくは「ゆれの大きさ」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（30, 0）。",
  ...
  lf_kokorikaext11_395: {
    ...
    steps: [
      "よこじくは「地震発生からの時間（秒）」、たてじくは「震源距離（km）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（40, 320）。",
      "全体として320だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
  ...
  lf_kokorikaext11_397: {
    ...
    steps: [
      "よこじくは「初期微動継続時間（秒）」、たてじくは「震源距離（km）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（30, 240）。",
      "全体として240だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
      "かたむきが急なところほど、変わり方が大きい。",
      "P波8km/s・S波4km/sのとき、初期微動継続時間Tと震源距離dの関係。d÷4−d÷8＝d÷8なのでT＝d÷8、つまりd＝8×Tとなり、原点を通る直線（比例）になる。T＝5秒なら40km、T＝10秒なら80km、T＝20秒なら160km、T＝30秒なら240km。比例定数8の単位はkm/秒である",
    ],
    buildSteps: 4,
  ...
  lf_kokorikaext11_398: {
    ...
    steps: [
      "よこじくは「10時23分00秒からの経過秒数（秒）」、たてじくは「震源距離（km）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（40, 0）、終わりは（70, 240）。",
      "全体として240だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
      "線が2本あるので、同じよこじくの値どうしで上下を見比べる。",
      "横軸を「時刻」にした走時グラフ。2直線の交点は震源距離0kmの点、すなわち地震が発生した瞬間を表す。交点は40秒の位置なので、地震発生時刻は10時23分40秒。震源距離80kmの地点では、P波が50秒（10時23分50秒）、S波が60秒（10時24分00秒）に到達する",
    ],
  ...
  lf_kokorikaext11_400: {
    ...
    steps: [
      "よこじくは「地震発生からの時間（秒）」、たてじくは「震源距離（km）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（25, 200）。",
      "全体として200だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
      "線が2本あるので、同じよこじくの値どうしで上下を見比べる。",
      "3地点A（震源距離40km）・B（120km）・C（200km）の到達時間をプロットした走時グラフ。P波は40÷8＝5秒、120÷8＝15秒、200÷8＝25秒で到達。S波は40÷4＝10秒、120÷4＝30秒、200÷4＝50秒で到達。初期微動継続時間はそれぞれ5秒・15秒・25秒となり、いずれも震源距離÷8に一致する",
    ],
    buildSteps: 4,
  ...
  lf_kokorikaext11_402: {
    ...
    steps: [
      "よこじくは「マグニチュード」、たてじくは「エネルギー（M5を1としたときの比）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「M7」で1000（エネルギー（M5を1としたときの比））。",
