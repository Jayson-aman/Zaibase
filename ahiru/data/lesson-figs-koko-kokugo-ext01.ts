...
export const lessonFigsKokoKokugoExt01: Record<string, Figure> = {
  ...
  lf_kokokokugoext01_001: {
    ...
    steps: [
      ...
      "いちばん大きいのは「常用漢字」で2136（字数）。",
      "いちばん小さいのは「小学校」で1026。差は1110。",
      "大きい順にならべると 常用漢字 → 中学校 → 小学校。棒の高さを見比べて確かめる。",
      "入試の書き取りが対象とする範囲。小学校で習う1026字と中学校で習う1110字を合わせて、常用漢字2136字になる。範囲は有限なので、三年かけて確実につぶせる量である",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokokokugoext01_007: {
    ...
    steps: [
      ...
      "いちばん大きいのは「音＋音」で、全体のおよそ68%。",
      "内わけは 音＋音 68%、訓＋訓 20%、重箱読み 7%、湯桶読み 5%。",
  ...
  lf_kokokokugoext01_009: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「修飾」で、全体のおよそ35%。",
      "内わけは 修飾 35%、似た意味 30%、目的・補語 15%、反対 12% …。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "二字熟語の構成のおおよその内訳。修飾と似た意味の重ねで六割以上を占める。判定が最も速い「反対」から先に処理し、残りを返り読みで振り分けるのが手順",
    ],
    buildSteps: 4,
  ...
  lf_kokokokugoext01_012: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「二字＋一字」で、全体のおよそ45%。",
      "内わけは 二字＋一字 45%、一字＋二字 40%、一字×3の並立 8%、三字で一語 7%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "三字熟語の構成のおおよその内訳。ほとんどが「二字＋一字（積極的・可能性）」か「一字＋二字（大自然・不本意）」なので、切れ目が上か下かを見きわめれば大半は判定できる",
    ],
    buildSteps: 4,
  ...
  lf_kokokokugoext01_015: {
    ...
    steps: [
      ...
      "印がついているのは 一朝一夕、四面楚歌、七転八倒、十人十色。位置を目もりで確かめる。",
      "数字を含む四字熟語を、その数字の位置に並べた図。数字入りの語は「一」を含むものが最も多い。数字は正確な数ではなく「わずか」「たくさん」を表すことが多い（千載一遇の千など）",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokokokugoext01_017: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は-600から0まで、100きざみで目もりがふってある。",
      "印がついているのは 温故知新（論語）、臥薪嘗胆（呉越）、五十歩百歩（孟子）、四面楚歌（垓下）。位置を目もりで確かめる。",
      "故事成語のもとになったおおよその年代（マイナスは紀元前）。春秋時代の論語から、戦国時代の孟子、そして漢が楚を破った垓下の戦いまで、五百年ほどの中国史の中で生まれている",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokokokugoext01_021: {
    ...
    steps: [
      "よこじくは「体の部分」、たてじくは「慣用句の数（目安）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「目」で40（慣用句の数（目安））。",
      "いちばん小さいのは「耳」で18。差は22。",
      "大きい順にならべると 目 → 手 → 口 → 足 → 頭 → 耳。棒の高さを見比べて確かめる。",
      "体の部分を使った慣用句の数のおおよその目安。目と手が突出して多い。見ること（判断）と働くこと（行動）が、人の生活の中心だったからだと考えられる",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokokokugoext01_024: {
    ...
    steps: [
      ...
      "印がついているのは 善は急げ、思い立ったが吉日、急がば回れ、待てば海路の日和あり。位置を目もりで確かめる。",
      "左へ行くほど「すぐ動け」、右へ行くほど「あせらず待て」。正反対の教えが両方ことわざとして残っているのは、ことわざが絶対の真理ではなく、場面に応じて使い分ける知恵だからである",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokokokugoext01_027: {
    ...
    steps: [
      "よこじくは「出典」、たてじくは「語数（目安）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「史記」で12（語数（目安））。",
      "いちばん小さいのは「列子」で4。差は8。",
      "大きい順にならべると 史記 → 韓非子 → 戦国策 → 論語 → 孟子 → 列子。棒の高さを見比べて確かめる。",
      "代表的な故事成語およそ60語を出典別に数えた目安。歴史書である史記が最も多い（四面楚歌・背水の陣・完璧）。韓非子・戦国策・列子は寓話、論語・孟子は思想家の言葉が出典になる",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokokokugoext01_030: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「不」で、全体のおよそ55%。",
      "内わけは 不 55%、無 25%、未 12%、非 8%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "打消の接頭語がつく語の内訳のおおよその目安。「不」が半分以上を占めるが、だからといって何にでも付けてよいわけではない。まず「まだ」が付くか（未）を試すのが判定の第一歩",
    ],
    buildSteps: 4,
  ...
  lf_kokokokugoext01_034: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「漢語」で、全体のおよそ47%。",
      "内わけは 漢語 47%、和語 35%、外来語 10%、混種語 8%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "国語辞典の見出し語に占める語種のおおよその割合。語の数では漢語が最も多い。ただし日常会話で実際に使う回数では和語のほうが多く、「数」と「使用頻度」は一致しない",
    ],
    buildSteps: 4,
  ...
  lf_kokokokugoext01_040: {
    ...
    steps: [
      "よこじくは「出題形式」、たてじくは「割合（％・目安）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「漢字読み書き」で45（割合（％・目安））。",
      "いちばん小さいのは「四字熟語」で10。差は35。",
      "大きい順にならべると 漢字読み書き → 語句の意味 → 慣用句・ことわざ → 四字熟語 → 熟語の構成。棒の高さを見比べて確かめる。",
      "語彙分野の出題形式別のおおよその割合。半分近くが漢字の読み書きなので、同音異義語の判別と送りがなを固めるのが最も効率がよい。合計は100％になる",
    ],
    buildSteps: 4,
  },
