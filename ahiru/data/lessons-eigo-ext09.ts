...
export const eigoExt09Lessons: Lesson[] = [
  //  1. 不定詞 名詞的用法（s296〜s299） 
  {
    ...
    intro: '「わたしは泳ぐのが好きです」を英語にするとき、I like swim. とは書けません。like のあとに動詞をそのまま置けないからです。英語では動詞を文の途中に入れるとき、to を付けて「動詞ではない何か」に変える必要があります。その変身のしかたを見ていきます。',
    order: 796,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：日本語訳だけで決めようとすると失敗する。「〜するために」と訳せても、その部分を消して文が成り立たないなら名詞的用法である。`,
...

  {
    ...
    intro: '「何か飲みたい」を I want drink. と言っても通じません。英語は動詞を二つ続けて並べられないからです。あいだに to を一つ入れて I want to drink. とすると、とたんに正しい文になります。この to のはたらきと、いっしょに使う動詞を身につけます。',
    order: 797,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：would like のあとの動詞は必ず to を付ける。would like go とはできない。また Would you like to 〜? は「〜しませんか」という誘いなので、Do you want to 〜? よりていねいだと覚えておく。`,
...

  {
    ...
    intro: '「医者になりたい」も「医者になろうと決めた」も「医者になろうとした」も、英語ではどれも「動詞＋to＋原形」で作れます。ちがうのは前の動詞だけです。want の形をひとつ覚えれば、hope・decide・try に入れかえるだけで言えることが一気に増えます。',
    order: 798,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：I hope you to come. は誤り。hope は「人＋to〜」の形をとらない。I want you to come.（あなたに来てほしい）の want とは形がちがうので混同しないこと。`,
...

  {
    ...
    intro: '「サッカーをすることは楽しい」の主語は、「サッカーをすること」という長いかたまりです。英語でもそのまま先頭に置けて、To play soccer is fun. となります。ではこのとき動詞は is か are か——迷いやすいところです。主語や補語になる不定詞を整理します。',
    order: 799,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：To play tennis is fun. は文法的に正しいが、実際の英文では It is fun to play tennis. の方がずっとよく使われる。英作文ではどちらも正解になるが、書きかえ問題では必ず It の形を求められる。`,
  ...
  //  2. 不定詞 形容詞的用法（s300〜s303） 
  {
    ...
    intro: '「何か冷たい飲み物」を英語にするとき、cold something drink のように前から並べたくなります。ところが正しくは something cold to drink。-thing で終わる語だけは、説明の語がすべてうしろに回ります。日本語と逆になるこの並び方を身につけます。',
    order: 800,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：I don\'t have nothing to do. のように否定を二つ重ねてはいけない。英語では否定は文に一つだけである。「何もない」は don\'t 〜 anything か nothing のどちらか一方で表す。`,
...

  {
    ...
    intro: '「やるべき宿題」「読む本」「話す時間」——日本語ではどれも説明が名詞の前に来ます。英語ではこれが逆で、homework to do、books to read、time to talk と、必ず名詞のうしろに置きます。この一つの型で、言えることがぐんと増えます。',
    order: 801,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：the first to arrive は「最初に到着した」であり、「最初に到着するつもり」ではない。文全体の時制（was／is）で、いつのことかが決まる。`,
...

  {
    ...
    intro: 'a big house は「大きい家」で前から、a house to live in は「住む家」でうしろから説明します。同じ家の説明なのに、なぜ位置がちがうのでしょう。実は語数で決まっていて、この原則は不定詞にも分詞にも関係代名詞にも共通します。',
    order: 802,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：「後ろから説明する」形に慣れていないと、the book to read を「本を読む」のように前から順に読んでしまい、文の意味を取りちがえる。名詞と説明のかたまりは必ずセットで囲むくせをつける。`,
...

  {
    ...
    intro: '「いっしょに遊ぶ友だち」を a friend to play で止めると、英語としては足りません。「友だちと遊ぶ」は play with a friend なので、with が必要なのです。文の最後にぽつんと前置詞が残るふしぎな形の理由を、もとの文に戻して確かめます。',
    order: 803,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：前置詞は必ず文の最後に置き、to の前には出さない。a friend with to play とは絶対に書かない。`,
  ...
  //  3. 不定詞 副詞的用法（s304〜s307） 
  {
    ...
    intro: '「なぜ公園へ行ったの？」に「サッカーをするために」と答えたいとき、英語では To play soccer. のひとことで足ります。because を使わなくてよいのです。動作の目的を to＋動詞の原形で表すこの言い方を、Why への答え方とあわせて練習します。',
    order: 804,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：日本語の「〜するように」につられて、for を使って for pass the exam などとしてはいけない。前置詞 for のあとに動詞の原形は置けない。目的を表すのは to＋原形か、for＋名詞（for the exam）である。`,
...

  {
    ...
    intro: 'I am glad. だけでは「うれしい」で終わってしまいます。「あなたに会えてうれしい」と理由まで言うには、うしろに to see you を足して I am glad to see you. とします。感情のことばのあとに続く不定詞が、その気持ちの原因を表すしくみを見ます。',
    order: 805,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：I am glad to see you. の to see you を「あなたに会うために」と訳すと「あなたに会うためにうれしい」となって意味が通らない。感情の形容詞のあとでは必ず「〜して」と訳す。`,
...

  {
    ...
    intro: 'He grew up to be a doctor. を「医者になるために育った」と訳すと、なんだか変です。正しくは「成長して医者になった」。同じ to＋原形でも、前から順に訳す用法があるのです。訳し方でつまずきやすい結果と判断の根拠を整理します。',
    order: 806,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：He is kind to help me. の to help me を「助けるために」と訳すと「助けるために親切だ」となって意味が通らない。形容詞が感情か性質かを先に確かめる習慣をつける。`,
...

  {
    ...
    intro: '不定詞の形はどれも to＋動詞の原形で、見た目はまったく同じです。それなのに入試では用法を答えさせられます。手がかりは訳し方ではなく、その不定詞を消したときに文が壊れるかどうか。三つの用法を機械的に見分ける手順をつくります。',
    order: 807,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：It is easy to swim. の to swim は形式主語 It が指す本当の主語なので名詞的用法である。「泳ぐために簡単だ」と副詞的に訳してはいけない。`,
  ...
  //  4. 疑問詞＋不定詞（s308〜s310） 
  {
    ...
    intro: '「自転車の乗り方を知っている」の「乗り方」は、英語では ride の名詞形ではありません。how to ride と二語をくっつけて、名詞のかたまりにします。「〜のしかた」をこの形で言えるようになると、説明する文が一気に書きやすくなります。',
    order: 808,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：how to のあとに to を重ねてしまうミスが多い。how to to get とは書かない。また get to the station の to は前置詞なので、how to get to 〜 と to が二回出てくるのは正しい形である。`,
...

  {
    ...
    intro: 'how to だけ覚えても、「何を買えばいいか」「いつ出発すればいいか」は言えません。how のところを what・when・where・which に取りかえるだけで、そのまま通じる形になります。四つの疑問詞のちがいと、意味の変わり方をまとめます。',
    order: 809,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：what to do の to のあとに主語を入れて what to I do とは書かない。主語を入れたいなら what I should do のように should の形にする。`,
...

  {
    ...
    intro: 'I don\'t know what to do. と I don\'t know what I should do. は、ほぼ同じ意味です。入試では片方をもう片方に書きかえさせます。このとき what のあとを疑問文の語順にしてしまう人が多いので、語順の決まりをここで固めます。',
    order: 810,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：I don\'t know what does he want. は誤り。文の中に入った疑問文では does を使わず、I don\'t know what he wants. とする。この語順のまちがいは入試で最も多い減点箇所の一つである。`,
  ...
  //  5. It is ... to 〜／too 〜 to／〜 enough to（s311〜s315） 
  {
    ...
    intro: 'It is easy to swim. の It は「それ」ではありません。訳すと「泳ぐことは簡単だ」で、It にあたる日本語がどこにもないのです。英語は長い主語を先頭に置くのをきらい、軽い It を先に立てて本体をうしろへ回します。その事情を見ていきます。',
    order: 811,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：It is easy swim. のように to を落とさない。また It is easy to swimming. のように to のあとを 〜ing にしない。`,
...

  {
    ...
    intro: 'It is easy to swim. は「だれにとって簡単なのか」を言っていません。「わたしには簡単だ」と付け加えるには、to の前に for me を入れます。ところが親切さを言う文では of you を使います。for と of の分かれ目をはっきりさせます。',
    order: 812,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：It is kind for you to help me. は誤り。性質を表す形容詞では必ず of を使う。この一問で差が付く出題が多い。`,
...

  {
    ...
    intro: 'This tea is too hot to drink. のどこにも not はありません。それでも意味は「熱すぎて飲めない」と否定になります。too という一語が「行きすぎ」を表し、その結果できない、という含みを持つからです。この形のしくみを見ます。',
    order: 813,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：too ... to 〜 の to のあとに目的語を重ねて書くのは誤り。主語がすでにその目的語だからである。`,
...

  {
    ...
    intro: '「彼は泳げるくらい強い」を英語にするとき、enough を strong の前に置きたくなります。しかし正しいのは strong enough to swim で、形容詞のあとです。ところが名詞に付くときは enough money と前に置きます。位置の決まりを整理します。',
    order: 814,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：enough を形容詞の前に置いて enough old とするのは誤り。名詞の前に置く enough money と混同しないこと。`,
...

  {
    ...
    intro: '入試では「ほぼ同じ内容の文に書きかえなさい」という問題がよく出ます。不定詞ではその材料が決まっていて、To 〜 is … と It is … to 〜、too … to 〜 と so … that … can\'t 〜 などのペアです。どちらからでも変換できるようにします。',
    order: 815,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：so ... that の that は接続詞なので省略できる場合もあるが、答案では必ず書く。省略すると採点者に伝わりにくい。`,
  ...
  //  6. 動名詞の用法（s316〜s319） 
  {
    ...
    intro: '不定詞で「〜すること」が言えるようになったのに、英語にはもう一つ同じ意味の形があります。動詞に ing を付けた動名詞です。二つもあるのはむだに見えますが、入る場所も相性のよい動詞もちがいます。まずは動名詞が置ける四か所を確かめます。',
    order: 816,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：Swimming is my favorite sport. の Swimming は動名詞の主語である。「泳いでいる」と進行形に訳さないこと。`,
...

  {
    ...
    intro: '「写真をとることが趣味です」を英語で言うとき、My hobby is take pictures. とはできません。is のあとに動詞をそのまま置けないからです。taking に変えれば通じます。文の先頭と be動詞のあと、この二か所での使い方を練習します。',
    order: 817,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：My hobby is play tennis. は誤り。be動詞のあとに動詞の原形は置けない。playing（動名詞）か to play（不定詞）にする。`,
...

  {
    ...
    intro: 'I enjoyed to swim. は、言いたいことは伝わっても英語としては誤りです。enjoy はうしろに to を置けず、swimming としか結びつきません。なぜ動詞によって相手が決まっているのか——まずは動名詞としか組まない代表的な動詞を覚えます。',
    order: 818,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：before・after・without・by・for・at・about・of はすべて前置詞である。これらのあとに動詞を置くときは 〜ing にする、と丸ごと覚える。`,
...

  {
    ...
    intro: '「歌が得意です」は I am good at sing. ではありません。at は前置詞なので、うしろには名詞しか置けず、singing に変える必要があります。この決まりが分かると、be good at・be interested in・without などが同じ理屈で片づきます。',
    order: 819,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：Thank you for to help me. は誤り。for は前置詞なので Thank you for helping me. とする。`,
  ...
  //  7. 不定詞だけ／動名詞だけを目的語にとる動詞（s320〜s323） 
  {
    ...
    intro: 'to と ing のどちらを置くかは、その場で考えても答えは出ません。うしろに来る形は、前の動詞ごとに決まっているからです。英語の動詞は「to だけ」「ing だけ」「どちらでもよい」の三組に分かれます。全体の地図をここで作ります。',
    order: 820,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：need は「必要とする」の意味では不定詞をとる（I need to sleep.）。物を主語にした need 〜ing（The car needs washing.）は高校で学ぶ形なので、中学受験では不定詞で覚えておけばよい。`,
...

  {
    ...
    intro: 'enjoy・finish・stop・practice——この仲間に共通するのは、すでに始まっていること、実際にしていることを扱う点です。だからうしろは ing になります。丸暗記の前にまとまりで感じをつかんでおくと、選びまちがえが減ります。',
    order: 821,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：Would you mind to open the window? は誤り。mind は動名詞専用なので opening にする。`,
...

  {
    ...
    intro: 'want・hope・decide・promise・plan——これらはどれも、まだしていないこれからのことを表します。だからうしろは、これから向かう感じのある to＋原形になります。あわせて want 人 to 〜 のような、人をはさむ形も確かめます。',
    order: 822,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：told me to not go out のように not を to のあとに置くのは誤り。not は必ず to の前である。`,
...

  {
    ...
    intro: 'He stopped to smoke. と He stopped smoking. は、たった一語のちがいで意味が正反対になります。前者はたばこを吸うために立ち止まり、後者はたばこをやめたのです。to はこれから、ing はもうしたこと。時間の向きで整理します。',
    order: 823,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：stop to 〜 の to＋原形は目的語ではないので、「〜することをやめる」という意味には絶対にならない。「やめる」と言いたいときは必ず stop 〜ing である。`,
  ...
  //  8. 現在分詞・過去分詞の形容詞用法（s324〜s327） 
  {
    ...
    intro: '「眠っている赤ちゃん」を a baby sleeping と言いたくなりますが、ふつうは a sleeping baby です。ing の形はもとは動詞なのに、一語のときは形容詞と同じように名詞の前に立ちます。動詞が名詞を説明する側に回る使い方を見ます。',
    order: 824,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：The boy running in the park is my brother. を「少年は公園で走っている」と読むと、後ろの is my brother が宙にういてしまう。動詞が二つあるように見えたら、前の 〜ing は説明のかたまりだと考える。`,
...

  {
    ...
    intro: 'a broken window の broken は、こわれた結果の状態を表しています。窓は自分でこわれたのではなく、こわされた側です。ing が「している」なら、過去分詞は「される・された」。受け身の意味で名詞を説明する形をおさえます。',
    order: 826,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：I know a boy called Ken.（ケンと呼ばれている少年を知っている）のように、called＝「〜と呼ばれる」の形は入試頻出である。call－called－called と変化する規則動詞である。`,
...

  {
    ...
    intro: 'the boy calling my name と the boy called Ken。前者は名前を呼んでいる少年、後者はケンと呼ばれている少年です。ing か過去分詞かを決めるのは、説明される名詞が「する側」か「される側」かの一点だけ。この判断を練習します。',
    order: 827,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：I am interesting in music. は誤り。「わたしが興味を持っている」のだから I am interested in music. とする。この一語のちがいで意味が正反対になる。`,
  ...
  //  9. 関係代名詞 who / which / that（s328〜s332） 
  {
    ...
    intro: '「わたしには京都に住んでいるおじがいます」を英語にすると、二つの文に分かれてしまいます。I have an uncle. He lives in Kyoto. これを一文にするために英語が用意したのが関係代名詞です。He を who に変えるだけでつながります。',
    order: 828,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：I know a boy who he plays the piano. は誤り。who がすでに主語のはたらきをしているので、he を重ねて書いてはいけない。`,
...

  {
    ...
    intro: '先行詞が人なら who、では犬やバスのときはどうするのでしょう。英語は人とそれ以外を区別するので、もの・動物には which を使います。that はそのどちらにも使える便利な語です。使い分けの境目を、例文で確かめていきます。',
    order: 829,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：I have a dog which it has long ears. は誤り。which が主語のはたらきをしているので it は不要である。`,
...

  {
    ...
    intro: 'the book which I read yesterday を見ると、read のうしろに目的語がありません。ふつうなら「何を読んだのか」が必要なはずです。実はその抜けた場所こそ、先行詞 the book が入るべき穴です。この穴に気づけると関係詞は読めます。',
    order: 830,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：目的格の関係代名詞は省略できる（次の課で学ぶ）。省略された文を読めるようにしておくことが、長文読解では決定的に重要である。`,
...

  {
    ...
    intro: '「昨日会った男の人」は the man whom I met yesterday ですが、実際の会話ではふつう the man I met yesterday と言います。whom は正式すぎて、日常ではあまり使われないのです。教科書の形と実際の形、その両方を知っておきます。',
    order: 831,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：who・whom・which・that はどれも「だれ・どれ」とは訳さない。関係代名詞は疑問詞ではないので、訳語を当てずに「後ろから説明するかたまり」として処理する。`,
...

  {
    ...
    intro: 'that はどんな先行詞にも使える万能選手ですが、逆に that でなければならない場面もあります。the first・the only・the best、all や every が付くときです。なぜそこだけ that なのかを、意味の面から考えてみます。',
    order: 832,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：接続詞の that は省略できる（I think he is right.）が、主格の関係代名詞の that は省略できない。省略できるのは目的格の関係代名詞である。`,
  ...
  //  10. 関係代名詞の省略・関係副詞（s333〜s336） 
  {
    ...
    intro: 'I like the book you gave me. は、名詞のあとにいきなり「主語＋動詞」が続くふしぎな形です。これは目的格の関係代名詞が省かれた跡です。省けるものと省けないものがあり、その線引きは形の上ではっきり決まっています。',
    order: 833,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：The man I met was kind. を「男の人はわたしに会った」と読むと was kind が宙にういてしまう。名詞のあとに突然別の主語が現れたら、省略された関係代名詞を補って読む。`,
...

  {
    ...
    intro: '長文でつまずく原因の多くは、単語ではなく「文の本当の動詞がどれか分からない」ことです。名詞のうしろに不定詞・分詞・関係詞節がくっつくと、動詞らしきものが二つも三つも並んで見えます。かたまりを切り分ける練習をします。',
    order: 834,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：説明のかたまりの中にある動詞（study・gave・standing）を文全体の動詞とまちがえると、訳が根本から狂う。動詞が複数あるときは、必ずどれが骨組みかを決めてから訳す。`,
...

  {
    ...
    intro: 'This is the house where he lived. の where を which に変えると誤りになります。lived は目的語を必要としない動詞で、うしろの文には抜けがないからです。「穴」があるかないかで、関係代名詞と関係副詞が分かれます。',
    order: 835,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：関係副詞のあとの文には、場所や時を表す語（there・then）を残してはいけない。This is the house where I was born there. は誤りである。`,
...

  {
    ...
    intro: '入試では空所に who・which・that・where・when のどれを入れるかが問われます。当てずっぽうではなく、先行詞は何か、うしろの文は完全か——この二つを順に確かめれば必ず一つに決まります。その手順を固めます。',
    order: 836,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：もとの語（She・it・there）を消し忘れるのが最も多いミスである。関係詞に置きかえた語は必ず消す。`,
      ...
      {
        heading: 'なぜ「あとの文が完全か」で関係代名詞と関係副詞が分かれるのか',
        mangaId: 'eigo_s336_manga',
        body: `関係代名詞（who・which・that）は、もとの文の**主語や目的語だった名詞**を置きかえた語です。だから関係代名詞のあとの文には、主語か目的語が欠けています（can play the guitar には主語がない）。関係副詞（where・when）は、もとの文の**there・then のような副詞**を置きかえた語なので、あとの文には主語も目的語もそろっています（we have lunch は完全）。つまり「あとの文に欠けた所があるか」を見れば、どちらの仲間かが決まり、欠けているのが主語なら主格、目的語なら目的格と分かります。

二文を一文にするときに、もとの語（She・it・there）を消し忘れる誤りが多いのは、関係詞が**その語の代わり**になっていることを意識していないからです。関係詞を置いたら、置きかえた語は必ず消します。前置詞が残る場合（play in it→which I often play in）は in を文末に残すか、in it をまとめて where にします。

■ 確かめのしかた
①**先行詞の種類**：人→who／that、もの→which／that、場所→where、時→when。
②**あとの文が完全か**：主語が欠けている→主格、目的語が欠けている→目的格、欠けていない→関係副詞。the day (　) I first met her→完全→when。
③二文を一文にするときは、共通する語を見つけ、あとの文でのはたらき（主語・目的語・副詞）を見て、関係詞に置きかえ、**もとの語を消す**。
④目的格は省略できる。目的格かどうかを確かめてから省略する。

★ ここがポイント：関係代名詞は**名詞の代わり**なのであとの文が不完全、関係副詞は**副詞の代わり**なのであとの文が完全。**先行詞→あとの文が完全か**の順に見て、置きかえた語は必ず消す。`,
      },
    ],
    trapExamples: [
      {
  ...
  //  11・12. 受動態（s337〜s340） 
  {
    ...
    intro: '「この写真はケンがとった」と「この写真はケンによってとられた」。同じ出来事でも、話の主役をどちらにするかで文の形が変わります。主役を入れかえるための道具が受動態です。語順がどう動くのかを、矢印で追いかけます。',
    order: 837,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：受動態の動詞部分に過去形を使うのは誤り。The window was broke by Tom. ではなく was broken である。過去形と過去分詞のちがいを必ず確認する。`,
...

  {
    ...
    intro: 'English is spoken in many countries. には、だれが話すのかが書かれていません。「人々によって」はわざわざ言うまでもないからです。受動態は、する人よりされるものに目を向けたいときに選ばれます。あわせて過去分詞を確実にします。',
    order: 838,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：受動態の文で過去形を使ってしまうミスが非常に多い。「be動詞のあとは必ず過去分詞」と唱えて確認する。`,
...

  {
    ...
    intro: '受動態は be動詞＋過去分詞なので、否定文も疑問文も be動詞の文とまったく同じ作り方で片づきます。not は be動詞のあと、疑問文は be動詞を主語の前に出すだけです。新しく覚えることは、実はほとんどありません。',
    order: 839,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：will is sold・can is seen のように、助動詞のあとに is を置くのは誤り。助動詞のあとは必ず原形の be である。`,
...

  {
    ...
    intro: '「わたしは音楽に興味があります」は I am interested by music. ではなく in music です。受動態の形なのに by を使わない表現がいくつもあり、しかも入試でよく問われます。前置詞ごとにまとめて覚えてしまいましょう。',
    order: 840,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
