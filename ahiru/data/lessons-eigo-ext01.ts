...
export const eigoExt01Lessons: Lesson[] = [
  //  1. アルファベット（s001〜s004） 
  {
    ...
    intro: '「ローマ字なら書けるよ」という人でも、いざ英文を書くと japan や i のように小文字で書き始めてしまいます。日本語には大文字・小文字の区別がないので、知らなければ必ずやるまちがいです。どこで大文字を使うのか、ここで決着をつけます。',
    order: 501,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：日本語では「日曜日」も「私」もふつうの字なので、つい sunday、i と書いてしまう。書いたあとに「文の頭・名前・曜日・月・I」の5つを見直す習慣をつける。`,
...

  {
    ...
    intro: 'ノートの4本線は、なんとなく引いてある飾りではありません。g や p を線の下までのばさずに書くと、q や 9 と見分けがつかなくなります。小文字の高さは3つのグループに分かれていて、そのきまりを知るだけで字が急に読みやすくなります。',
    order: 502,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：apple の p を「まん中の部屋」だけに書くと、a と同じ高さになってしまい、まったく別の字に見える。高さの情報も文字の形の一部だと考える。`,
      ...
      {
        heading: 'なぜ高さで文字が決まるのか',
        mangaId: 'eigo_s002_manga',
        body: `英語の小文字は、**形だけでなく高さも文字の情報**です。n と h は、たての棒が第1線まで届くかどうかだけのちがいで、a と d も同じです。p と n は丸の位置は似ていますが、棒が第4線まで下りるかどうかで区別します。だから高さをそろえて書かないと、読む人にはちがう文字に見え、正しく書いたつもりでも不正解になります。4本線は飾りではなく、この高さの情報を書き分けるための線です。

小文字を3つのグループに分けるのは、26字を1字ずつ覚えるより、**「上に出る7字・下に出る5字・残りはまん中」**と数で覚えるほうが確実だからです。上に出る7字のうち t だけ少し低いのは、l と同じ高さまでのばすと横棒が無いときに l と見分けがつかないからです。

■ 確かめのしかた
①書いた単語の**文字の下端が第3線にそろっているか**を見る。ここがそろえば多少形がくずれても読める。
②上に出る字（b d f h k l t）が第1線まで届いているか、t は少し低いか。
③下に出る字（g j p q y）が第4線まで下りているか。apple の p がまん中だけになっていたら a と同じ高さ→×。
④i・j の点、t の横棒は、たての線を書いてから最後に付けたか。

★ ここがポイント：小文字は**高さも文字の情報**（n と h、p と n は高さでしか区別できない）。**上に出る7字・下に出る5字・残りはまん中**と数で覚え、下端を第3線にそろえる。`,
      },
    ],
    trapExamples: [
      {
...

  {
    ...
    intro: '辞書も名簿も目次も、すべて A から Z の順に並んでいます。順番があやふやだと、辞書を引くだけで時間を取られます。しかも英語の Z は「ゼット」とは読みません。文字の名前と、文字が単語の中で出す音のちがいもここで整理します。',
    order: 503,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：文字の名前（エイ、ビー、シー）と、単語の中でその文字が出す音（ア、ブ、ク）はまったく別のもの。次の課からは「音」のほうを学ぶ。`,
      ...
      {
        heading: 'なぜ順番を速く言えることが大事で、なぜ名前と音を分けるのか',
        mangaId: 'eigo_s003_manga',
        body: `辞書も名簿も索引も、A から Z の順に並んでいます。順番があやふやだと、辞書で1語を引くたびに最初から歌わなければならず、時間を取られます。「R の2つ前は何か」のような問題も、順番そのものを問うています。ABC の歌の4つの区切り（7・9・6・4字）で覚え、M（13番目）を真ん中の目印にすると、途中からでも数えられるようになります。

文字の**名前**（A＝エイ、B＝ビー）と、単語の中でその文字が出す**音**（a＝ア、b＝ブ）を分けるのは、両方を混ぜると cat を「ケイト」と読んでしまうからです。名前は聞き取り問題やつづりを言うときに使い、音は単語を読むときに使います。日本語の「ゼット」「エル・アール」の区別も、英語の名前とはちがうので、英語の音で覚え直します。

■ 確かめのしかた
①26字を**何も見ずに10秒以内**で書けるか。書けないなら区切り（ABCDEFG／HIJKLMNOP／QRSTUV／WXYZ）ごとに練習する。
②「〇の2つ前」は、指でアルファベット表を**左にたどる**。R の2つ前→Q→P。「前」と「後ろ」の方向をまちがえていないか。
③まぎらわしい名前を対で言えるか：G（ジー）／J（ジェイ）、M（ム）／N（ヌ）、E（イー）／I（アイ）、V／B、Z（ズィー）。
④名前と音を混ぜていないか。cat の a は「エイ」ではなく短母音の音。

★ ここがポイント：A〜Z の順番は**辞書引きの速さ**そのものなので、区切りと M の目印で途中からも数えられるようにする。**文字の名前と音は別もの**で、名前は聞き取りとつづり、音は読むときに使う。`,
      },
    ],
    trapExamples: [
      {
...

  {
    ...
    intro: '答案に書いた u が v に見えたら、内容が合っていても不正解になります。日本語ではめったに起きませんが、英語ではこれが本当に起こります。入試で通用する字の形と、まぎらわしい字の書き分けを確かめましょう。',
    order: 504,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：答案を書き終えたら、自分の書いた単語を「知らない人が読んだらどう見えるか」という目で読み返す。あやしい字は消して書き直したほうが安全である。`,
  ...
  //  2. 母音字・子音字と短母音（s005〜s010） 
  {
    ...
    intro: 'by や my には母音字が一つも見当たりません。それなのに読めるのはなぜでしょうか。実は y が母音の役をしているからです。26文字を母音字と子音字に分けて考えると、a と an の使い分けまで見通せるようになります。',
    order: 505,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：「a・i・u・e・o で始まる語には an」と覚えると、university や hour でまちがえる。必ず声に出して、最初の音で判断する。`,
...

  {
    ...
    intro: '知らない語に出会ったとき、日本語なら漢字から意味の見当がつきます。英語ではつづりから音の見当をつけます。その手がかりになるきまりがフォニックスです。まずは cat・bed・sit の a・e・i、5つの短い母音から始めます。',
    order: 506,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：短母音のうち、日本人がとくに苦手なのは a と u の区別である。cat と cut、bag と bug、hat と hut は、母音の1音だけで意味が変わる。`,
      ...
      {
        heading: 'なぜフォニックスから始めるのか、なぜ CVC 型は短母音なのか',
        mangaId: 'eigo_s006_manga',
        body: `英語のつづりと音には、かなりの部分できまりがあります。このきまり（フォニックス）を知っていれば、初めて見る単語でも音の見当がつき、聞いた音からつづりを組み立てられます。知らなければ、単語を1つずつ丸暗記することになり、覚える量が何倍にもなります。だから最初に、いちばん基本の5つの短母音から始めます。

「子音＋母音字1つ＋子音」（CVC 型）で終わる語の母音が短母音になるのは、母音字が**1つだけで、あとに子音でふさがれている**からです。cat・bed・sit・box・cup はどれもこの形です。母音字が2つ並んだり（rain）、語末に e が付いたり（cake）すると、母音は長くのびて文字の名前の音（エイ）になります。つまり「母音字が1つで、うしろが子音」なら短く、それ以外なら別のきまり、と区別できます。

■ 確かめのしかた
①単語を c-a-t のように**音に分けて**、母音字が1つで子音にはさまれているか見る。そうなら短母音。
②5つの短母音を**口の形**で言えるか：a（横に大きく開く・エとアの中間）、e（軽く横に開く）、i（少し開いて弱く）、o（丸くたてに開く）、u（あまり開けず、のどの奥で短くア）。
③日本語の母音で代用していないか。cat を「ア」で言うと cut、sit を強く「シート」と言うと seat になる。
④聞き取りで bad と bed のように迷ったら、**意味の通る文**を作って確かめる（That is a bad idea. ／ I sleep in my bed.）。

★ ここがポイント：フォニックスは**つづりと音のきまり**で、知っていれば初めての単語も読める。**母音字が1つで子音にはさまれた語（CVC）は短母音**。5つの音は口の形で覚え、a と u の区別をとくに練習する。`,
      },
    ],
    trapExamples: [
      {
  ...
  },
  {
    ...
    intro: 'bad と bed はまん中の1文字だけがちがいます。ところがカタカナに直すとどちらも「ベッド」に近くなり、区別が消えてしまいます。pen と pin も同じです。音のちがいを、意味とセットで覚えていきましょう。',
    order: 507,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：日本語のカタカナ語がじゃまをすることがある。「デスク」「テン」はそのまま desk・ten でよいが、「ピン」は pin、「ペン」は pen と、カタカナでは同じ「エ／イ」に聞こえても英語では別音である。`,
...

  {
    ...
    intro: 'cat（ねこ）と cut（切る）、hat（ぼうし）と hut（小屋）。カタカナで書くとどれも「ア」ですが、英語では別の音です。日本語の「ア」は一つしかないので、ここは口の形から覚え直す必要があります。',
    order: 508,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：カタカナ語にひっぱられる例に注意する。「カップ」は cup だが「キャップ」は cap、「バッグ」は bag だが「バグ」は bug である。日本語になっている語こそ、つづりを確かめる。`,
...

  {
    ...
    intro: 'b と p、d と t は、口の形がまったく同じです。ちがうのは、のどをふるわせるかどうかだけ。この区別は、あとで学ぶ books と dogs の -s の読み分けを決めるときに必ず使います。今のうちに整理しておきましょう。',
    order: 509,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：これらは「日本語のどの音に近いか」ではなく「口のどこをどう使うか」で覚える。カタカナに直した瞬間に区別が消えることを忘れない。`,
...

  {
    ...
    intro: 'c・a・t の3つの音をつなげると cat になります。逆に cat という音を聞いたら、c／a／t と3つに切り分けて書けます。この行き来ができるようになると、習っていない語でも読めて書けるようになります。',
    order: 510,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：CVC のきまりが当てはまらない語もある。put（プット）の u は「ア」ではなく「ウ」に近い音、was（ワズ）の a も例外である。ただし、こうした例外は数が限られているので、まず原則を身につけてから覚えればよい。`,
  ...
  //  3. 長母音・マジックE・二重母音（s011〜s016） 
  {
    ...
    intro: 'cap は「キャップ」、cape は「ケイプ」。最後に e が付いただけで、まん中の a が文字の名前そのままの「エイ」に変わりました。この「文字の名前を言う音」が長母音です。長母音になる合図は3つあります。',
    order: 511,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：音が同じでつづりがちがう語（同音異義語）は入試で頻出。meet（会う）／meat（肉）、see（見る）／sea（海）、right（右）／write（書く）、know（知る）／no（いいえ）。意味とセットで覚える。`,
...

  {
    ...
    intro: 'hop は「ぴょんと跳ぶ」、hope は「望む」。読まない e が1つ付くだけで、音も意味も変わってしまいます。この e は自分では音を出さないのに前の母音を変えるので、マジックE と呼ばれます。しくみを知れば読み分けられます。',
    order: 512,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：makeing、writeing のように e を残したまま ing を付けるのは、書き取りでいちばん多いまちがい。「e を取って ing」と口で唱えてから書く。`,
  ...
  },
  {
    ...
    intro: 'rain の ai は「アイ」ではなく「エイ」と読みます。ローマ字の感覚で読むとまず通じません。母音字が2つ並ぶと、前の字が名前を言い、後ろの字は黙る。これが基本です。ただし ea には例外が多く、そこが勝負どころです。',
    order: 513,
    studyPeriod: '小4',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：例外語は「読み」と「意味」をセットにして声に出して覚える。bread（パン）を「ブリード」と読んでしまうと、聞き取りでも書き取りでも通じない。`,
...

  {
    ...
    intro: 'boy を「ボイ」、house を「ハウス」と読めば通じそうに思えますが、英語では口の形が動きながら1つの音になります。しかも同じ音でも、語のとちゅうと語末ではつづりが変わります。その使い分けを整理します。',
    order: 514,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：know（知っている）と now（今）は、つづりが1字ちがうだけで読み方も意味もまったくちがう。know は k を読まないので「ノウ」、now は「ナウ」である。書き取りでよく混同する。`,
...

  {
    ...
    intro: 'book と school はどちらも oo なのに、「ブック」と「スクール」で長さがちがいます。walk の l に至っては読みません。ここでは oo の2つの読み方と、「オー」と読むつづりをまとめて片づけます。',
    order: 515,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：foot（足・単数）と feet（足・複数）と food（食べ物）を混同しないこと。つづりも意味もちがう3語である。`,
...

  {
    ...
    intro: 'car の ar、bird の ir、turn の ur。母音字のあとに r が来ると、短母音でも長母音でもない別の音に変わります。しかも er・ir・ur は音では区別できません。つづりで覚えるしかない、という割り切りが要る単元です。',
    order: 516,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：first（1番目）, third（3番目）, thirteen（13）, thirty（30）はすべて ir を使う。これらは数の語として入試頻出なので、まとめて確認する。`,
  ...
  //  4. 子音連結・二重子音字（s017〜s022） 
  {
    ...
    intro: 'blue を「ブルー」、play を「プレイ」と言ってしまうのは、日本語が「子音＋母音」でできているからです。英語は子音を続けて出せます。b と l のあいだに「ウ」を入れないこと。ここが日本人の最大の壁です。',
    order: 517,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：日本語のカタカナ語は l と r を区別しないので、「グラス」「フライ」といったカタカナ語からつづりを決めてはいけない。必ず意味から決める。`,
...

  {
    ...
    intro: 'street には s・t・r と子音が3つ続きます。カタカナの「ストリート」は母音が3つも入っていて、音の数がまるでちがいます。s で始まる連結を、あいだに母音をはさまずに出せるようにしていきましょう。',
    order: 518,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：書き取りでは、聞こえにくい語末の子音こそ落としやすい。「意味の通る形になっているか」を最後に確認する（friend の d、second の d を落とすと別の語になる）。`,
  ...
  },
  {
    ...
    intro: 'she の sh は、s と h を別々に読むわけではありません。2文字で1つの音です。では watch はなぜ tch と3文字も使うのでしょうか。実はここにも、直前の母音がからんだきまりがあります。まとめて片づけましょう。',
    order: 519,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：watch を wach、catch を cach と書くまちがいが非常に多い。「短い母音のあとの語末のチは tch」と唱えて覚える。`,
...

  {
    ...
    intro: 'phone には f が1文字もないのに「フォン」と読みます。ph が f の音になるからです。sing の ng も「ング」と2つに切ってはいけません。-ing の形はこれから何百回も出てくるので、今のうちに音を固めます。',
    order: 520,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：ing を「イ・ン・グ」と3つに切って発音・表記すると、日本語なまりが強くなる。「ング」をひとかたまりの鼻の音として練習する。`,
...

  {
    ...
    intro: 'think を「シンク」と言うと sink（流し）に聞こえます。they を「デイ」と言えば day です。th は日本語にない音なので、似た音で代用すると別の語になってしまいます。舌の位置ひとつで通じ方が変わります。',
    order: 521,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：th のあとに r が続く three, throw, through は、th と r という日本語にない音が2つ続く。とくにゆっくり練習する。`,
...

  {
    ...
    intro: 'what・when・where は wh を w と読むのに、who だけは h の音になり w を読みません。同じ疑問詞なのに不ぞろいです。また q は必ず u とセットで使います。例外はまとめて覚えてしまいましょう。',
    order: 522,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：quiet と quite は、i と t の順番が入れかわっているだけ。書くときも読むときも、この2語は必ず確かめる。`,
  ...
  //  5. サイレントレターとつづりの例外（s023〜s026） 
  {
    ...
    intro: 'know の k、write の w、climb の b。どれも書くのに読みません。聞いたとおりに書くと必ず落としてしまう文字です。一つずつ覚えるより、同じ形のなかまでまとめて覚えるほうがずっと速く済みます。',
    order: 523,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：聞き取り問題では、音が同じ語が答えの候補になることがある。前後の意味からどちらかを判断する練習をしておく。`,
...

  {
    ...
    intro: 'night の gh、walk の l、listen の t。書いてあるのに消える文字は、耳ではなく「知っているかどうか」で差がつきます。laugh のように gh を f と読む語まであります。ここで一気に整理します。',
    order: 524,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：Wednesday は曜日の中でいちばんつづりをまちがえやすい。「ウェド・ネス・デイ」と、書くとき専用の読み方をして覚えるとよい。`,
  ...
  },
  {
    ...
    intro: 'cat の c は「ク」、city の c は「ス」。同じ1文字なのに読み方が変わります。でたらめではなく、次に来る母音字が e・i・y かどうかで決まります。きまりを知れば、初めて見る語も読めます。',
    order: 525,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：g のきまりは例外が多いので、「あとが e・i・y ならジのことが多い」くらいに考え、get・give・girl は例外として先に覚えておく。`,
...

  {
    ...
    intro: '英語は26文字しかないのに、音は40以上あります。文字が足りないので、1つの文字がいくつもの音を担当することになりました。つづりどおりに読めないのはそのためです。理由が分かると、覚え方そのものが変わってきます。',
    order: 526,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：③と④を混同して makeing、runing と書くまちがいが非常に多い。「e があれば取る、短母音＋子音1つなら重ねる」と2つを並べて覚える。`,
  ...
  //  6. アクセント・リズム・語尾の発音（s027〜s031） 
  {
    ...
    intro: 'strike は日本語では「ス・ト・ラ・イ・ク」と5拍ですが、英語では1かたまりです。この数え方のずれが、英語が速く聞こえる正体の一つです。あごに手をあてて数えると、英語のリズムの単位が見えてきます。',
    order: 527,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：語末の -ed が音節を1つ増やすのは、t・d で終わる語のときだけである（want・ed は2音節、played は1音節）。次の課で学ぶ -ed の発音と関係している。`,
...

  {
    ...
    intro: '日本語では、語のどこを強く読むかをあまり意識しません。英語は強く読む場所がずれるだけで通じなくなります。しかも名詞と動詞でアクセントの位置が動く語まであります。どこを強く読むのか、そのきまりを見ていきましょう。',
    order: 528,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：アクセントの位置がずれると、正しい音を出していても通じないことがある。日本語で「はし」の高さを変えると意味が変わるのと同じくらい、英語では強さの位置が重要である。`,
...

  {
    ...
    intro: 'books は「ブックス」、dogs は「ドッグズ」、buses は「バスィズ」。同じ -s なのに読み方が3通りあります。しかも自分で選んでいるのではなく、直前の音が自動的に決めています。そのしくみを解きます。',
    order: 529,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：三単現の -s も同じきまりで動く（go → goes、watch → watches、study → studies）。複数形とセットで覚えれば、覚える量が半分になる。`,
...

  {
    ...
    intro: 'played は「プレイド」、looked は「ルックト」、wanted は「ウォンティド」。同じ -ed でも3通りに読み分けます。wanted だけは音のかたまりが1つ増えるので、聞き取りでも書き取りでも差がつきます。',
    order: 530,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：study は「子音＋y」なので studied、play は「母音＋y」なので played。y の1つ前の文字を必ず見る。この見分けは複数形・三単現でも同じである。`,
  ...
  },
  {
    ...
    intro: '英語が速く感じられるのは、話す人が急いでいるからではありません。a・the・of のような語を思い切り弱く読むからです。an apple が「アナポー」に聞こえるのもそのため。強弱のしくみが分かると急に聞き取れます。',
    order: 531,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：聞き取り問題では「単語が速すぎて聞こえない」と感じることが多いが、実際には弱く読まれてつながっているだけである。強く読まれる語だけを拾えば意味はつかめる。`,
  ...
  //  7. ローマ字と英語つづり（s032〜s035） 
  {
    ...
    intro: '「ふじ」は Huzi でしょうか、Fuji でしょうか。実はどちらも正しいローマ字です。学校で最初に習う訓令式と、パスポートや駅名で使うヘボン式の2種類があるからです。ちがいが出るのは、し・ち・つ・ふ・じの行だけです。',
    order: 532,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：訓令式で書いた「Huzisan」は英語話者にはまったく通じない。h－u と読むと「ヒュージサン」のようになってしまう。英語の中では必ず Fuji と書く。`,
...

  {
    ...
    intro: '「きっぷ」の小さい「っ」、「しんいち」の「ん」、「さとう」ののばす音。かなでは書けても、ローマ字ではどう書くか迷います。パスポートでは「さとう」は Sato と書きます。書き方のきまりを確かめましょう。',
    order: 533,
    studyPeriod: '小5前半',
    targetLevel: 'kiso',
    keyPoints: [
...
⚠ 注意：東京は Tokyo と書く。To-u-kyo-u ののばす音を書かないので、4文字分の音が Tokyo という6文字になる。Toukyou と書くのは入力用の書き方で、英語の文の中では使わない。`,
...

  {
    ...
    intro: 'ローマ字を先に覚えてから英語を習うと、like を「リケ」、name を「ナメ」と読んでしまいます。ローマ字は日本語を書くための道具で、英語のつづりのきまりとは別ものです。ここで頭を切りかえましょう。',
    order: 534,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：ローマ字は日本語を書くための道具であって、英語の読み方を教えてくれるものではない。「ローマ字ができるから英語も読める」と考えると、かえって読みまちがいが増える。`,
...

  {
    ...
    intro: '自己紹介で自分の名前を英語で書くとき、姓が先か名が先か迷いませんか。今の公用文では姓を先にするのが原則で、姓をすべて大文字にする書き方もあります。地名や「〜山」「〜駅」の書き方もここで決めておきます。',
    order: 535,
    studyPeriod: '小5前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：日本の地名は、英語の中では日本語の語をそのまま使う。「東京」は Eastern Capital などと訳さない。固有名詞は訳さずに音を写す、というのが原則である。`,
  ...
  //  8. つづりミスと書き取り（s036〜s040） 
  {
    ...
    intro: 'Wednesday の d、February の最初の r。どちらも発音ではほとんど聞こえないのに、書かなければ不正解です。曜日と月の名前は入試で必ず出るうえ、文のとちゅうでも大文字で書き始めます。ここで固めます。',
    order: 536,
    studyPeriod: '小5後半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：September から December は語尾が -ber でそろっている。September, October, November, December の4つをまとめて書く練習をすると定着が速い。`,
      ...
      {
        heading: 'なぜ読まない文字があり、なぜ大文字で始めるのか',
        mangaId: 'eigo_s036_manga',
        body: `Wednesday の d や February の最初の r は、発音ではほとんど聞こえません。これは、英語のつづりが**昔の発音のまま残っている**からです。Wednesday はもともと「ウォーデンの日」という意味で、発音が変わっても文字は変わりませんでした。だから音のとおりに書くと文字が落ちます。防ぐには、「ウェド・ネス・デイ」「フェブ・ル・アリー」のように**書く用の読み方**で覚えるのが確実です。

曜日・月の名前を文のとちゅうでも大文字で始めるのは、英語では**固有名詞（人名・地名・曜日・月）は大文字で始める**というきまりがあるからです。曜日や月は、神や皇帝の名前から来た固有名詞なので、on sunday と小文字で書くと不正解になります。曜日の前に on、月の前に in を使うのも、決まった組み合わせとして覚えます。

■ 確かめのしかた
①曜日・月を書いたら、**大文字で始まっているか**を見る。
②落としやすい文字を確かめる：Wednesday の d、Thursday の r、Saturday の u、February の最初の r、August の最後の t、October に余分な r を入れていないか。
③つづりを区切って読めるか：Tues＋day、Thurs＋day、Sat＋ur＋day、Feb＋ru＋ary。
④前置詞は on＋曜日（on Sunday）、in＋月（in June）、on＋日付（on June 10）。日付は序数で読む（June tenth）。

★ ここがポイント：つづりは**昔の発音のまま残っている**ので、聞こえない文字がある。**書く用の読み方**（ウェド・ネス・デイ）で覚え、曜日・月は**固有名詞なので大文字**で始める。on＋曜日、in＋月。`,
      },
    ],
    trapExamples: [
      {
  ...
  },
  {
    ...
    intro: 'favorite を聞こえたとおりに書くと favrite になります。chocolate も、まん中の母音が消えて聞こえます。英語には「読むときは消えるのに、書くときは要る母音」があるのです。書く用の読み方で覚えます。',
    order: 537,
    studyPeriod: '小5後半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：つづりを覚えるときは、必ず声に出しながら手で書く。目で見るだけでは定着しない。同じ語を3回書いて、4回目は見ないで書く、という順序が効率がよい。`,
...

  {
    ...
    intro: 'tomorrow の r、dinner の n。音を聞いても1つにしか聞こえないので、書くときに落としがちです。どこで文字を重ねるのかは丸暗記に見えますが、run から running のように理由のあるものもあります。',
    order: 538,
    studyPeriod: '小5後半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：hopping と hoping は1文字ちがいでまったく別の語になる。母音が短いか長いかを、重ねるかどうかで書き分けている。文字を重ねることには、音を短く保つという役目がある。`,
...

  {
    ...
    intro: 'four には u があるのに、forty には u がありません。nine には e があるのに、ninth では消えます。数のことばは毎日使うのに、つづりの変化がふぞろいです。まちがえやすい語から先につぶしておきましょう。',
    order: 539,
    studyPeriod: '小5後半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：nine → ninth と nineteen の e のあつかいがちがう。ninth では e が消え、nineteen では残る。まとめて書いて確かめる。`,
...

  {
    ...
    intro: '書き取りでいちばん多いミスは、聞こえなかった a や the の落としと、文末のピリオド忘れです。つまり耳よりも手順で差がつきます。3回に分けて書き進めるやり方と、書き終えたあとの6つの点検項目を身につけます。',
    order: 540,
    studyPeriod: '小5後半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：完ぺきに書き取れなくてもよい。強く読まれる語（意味の中心）が正しく取れていれば、内容は理解できている。まずそこを確実にする。`,
      ...
      {
        heading: 'なぜ耳より手順で差がつくのか',
        mangaId: 'eigo_s040_manga',
        body: `書き取りで落とすのは、聞こえなかった難しい語ではなく、a・the・to・of・-s のような**弱く短く読まれる部分**です。これらはどんなに耳がよくてもほとんど聞こえません。しかし文法で決まっているので、**知識で補える**部分でもあります。He play soccer. と聞こえても、主語が He で現在の文なら plays に決まります。two cat と聞こえても、two があれば cats に決まります。だから、3回に分けて聞き、最後に6つの点検をする手順のほうが、耳を鍛えるより確実に点が上がります。

1回目に書かずに意味をつかむのは、強く読まれる語（名詞・動詞・形容詞）が意味の骨組みで、それが分かれば弱い語を補う手がかりになるからです。聞こえない語を空白にして先へ進むのは、止まるとそのあとを全部聞きのがすからです。

■ 確かめのしかた
①1回目は書かずに聞き、**何についての文か**を言えるか。
②2回目は語を書き、聞こえない所は下線を引いて空白にする。
③3回目に**6つの点検**：文頭の大文字／固有名詞・I の大文字／ピリオドか ? ／三単現の s／複数形の s／過去形。
④知らない語はフォニックスで組み立てる（「キャット」→cat、「ネイム」→name）。最後に**声に出して読み返し**、意味が通るか。every day は2語。

★ ここがポイント：落とすのは**弱く読まれる語**で、それは**文法の知識で補える**。3回に分けて聞き、**6つの点検**を必ず行う。手順が耳を助ける。`,
      },
    ],
    trapExamples: [
      {
