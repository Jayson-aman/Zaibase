...
export const lessonFigsKokoShakaiExt11: Record<string, Figure> = {
  ...
  lf_kokoshakaiext11_372: {
    ...
    steps: [
      "よこじくは「候補者」、たてじくは「得票数（千票）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「A（当選）」で40（得票数（千票））。",
      "いちばん小さいのは「C（落選）」で25。差は15。",
      "大きい順にならべると A（当選） → B（落選） → C（落選）。棒の高さを見比べて確かめる。",
      "定数1の小選挙区の例。総投票100千票のうち当選したAは40千票で、B・Cに投じられた60千票（全体の60％）はすべて死票になる。当選者の得票が過半数に届かないことも起こる",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_373: {
    ...
    steps: [
      "よこじくは「議院と選挙のしくみ」、たてじくは「定数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「衆・小選挙区」で289（定数（人））。",
      "いちばん小さいのは「参・比例代表」で100。差は189。",
      "大きい順にならべると 衆・小選挙区 → 衆・比例代表 → 参・選挙区 → 参・比例代表。棒の高さを見比べて確かめる。",
      "衆議院465人＝小選挙区289＋比例代表176。参議院248人＝選挙区148＋比例代表100。青が衆議院、緑が参議院。足し算で必ず検算できるようにしておく",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_374: {
    ...
    steps: [
      "よこじくは「選挙区（いずれも定数1）」、たてじくは「議員1人あたりの有権者数（万人）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「A区」で48（議員1人あたりの有権者数（万人））。",
      "いちばん小さいのは「D区」で24。差は24。",
      "大きい順にならべると A区 → B区 → C区 → D区。棒の高さを見比べて確かめる。",
      "一票の格差＝48万÷24万＝2.0倍。有権者が少ないD区の一票がもっとも重く、多いA区の一票がもっとも軽い。「人数が多い区ほど一票は軽い」と直感の逆になる点に注意",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_375: {
    ...
    steps: [
      "よこじくは「年代」、たてじくは「投票率（％・おおよその値）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「60代」で71（投票率（％・おおよその値））。",
      "いちばん小さいのは「20代」で36。差は35。",
      "大きいほうから 60代 → 50代 → 70代〜 → 40代 と続く。上位がどれかをおさえる。",
      "衆議院議員総選挙における年代別投票率のおおよその傾向。20代が最も低く60代が最も高い。投票率の低い世代の要望は政策に反映されにくくなるという指摘がある",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_381: {
    ...
    steps: [
      "よこじくは「議院と選挙」、たてじくは「人数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「衆・定数」で465（人数（人））。",
      "いちばん小さいのは「参・通常選挙で改選」で124。差は341。",
      "大きい順にならべると 衆・定数 → 衆・総選挙で改選 → 参・定数 → 参・通常選挙で改選。棒の高さを見比べて確かめる。",
      "衆議院は総選挙で465人全員を選び直す（任期4年・解散あり）。参議院は任期6年で3年ごとに半数の124人だけを改選する（解散なし）。「6年ごとに全員を選び直す」は誤り",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_383: {
    ...
    steps: [
      ...
      "印がついているのは ①提出（議員・内閣）、②委員会、③本会議、④他院の委員会。位置を目もりで確かめる。",
      "法律ができるまでの流れ。「委員会→本会議」を衆参それぞれで1回ずつ、計4段階ふむのが骨格。両院で可決すれば成立し、内閣を経て天皇が公布する",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokoshakaiext11_384: {
    ...
    steps: [
      ...
      "内わけは 社会保障関係費 33%、国債費 24%、地方交付税交付金等 16%、防衛関係費 8% …。",
  ...
  lf_kokoshakaiext11_385: {
    ...
    steps: [
      ...
      "印がついているのは 内閣総理大臣の指名、予算・条約の承認、法律案。位置を目もりで確かめる。",
      "参議院が議決しないまま経過したとき衆議院の議決が優先される日数。首相の指名10日、予算・条約30日、法律案60日。決まらないと困る度合いが強いものほど日数が短い",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokoshakaiext11_389: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は0から80まで、10きざみで目もりがふってある。",
      "印がついているのは 不信任決議の可決、解散または総辞職、総選挙、特別会の召集。位置を目もりで確かめる。",
      "横軸は不信任決議の可決からの経過日数（最長の場合）。10日以内に解散か総辞職、解散なら解散の日から40日以内に総選挙、総選挙の日から30日以内に特別会。10→40→30の順",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokoshakaiext11_392: {
    ...
    steps: [
      ...
      "印がついているのは 第一審：地方裁判所、第二審：高等裁判所、第三審：最高裁判所。位置を目もりで確かめる。",
      "三審制の段階。第一審から第二審へは控訴、第二審から第三審へは上告という。第一審は事件により簡易裁判所・家庭裁判所になることもある。確定後のやり直しは再審であり第四審ではない",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokoshakaiext11_395: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「裁判員 6人」で、全体のおよそ66.7%。",
      "内わけは 裁判員 6人 66.7%、裁判官 3人 33.3%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "裁判員裁判の合議体は裁判員6人＋裁判官3人＝9人。有罪とするには裁判官1人以上を含む過半数の賛成が必要で、裁判員6人全員が有罪でも裁判官が全員無罪なら有罪にできない",
    ],
    buildSteps: 4,
  ...
  lf_kokoshakaiext11_403: {
    ...
    steps: [
      "よこじくは「請求の種類」、たてじくは「必要な署名数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「議会の解散」で20000（必要な署名数（人））。",
      "いちばん小さいのは「条例の制定・改廃」で1200。差は18800。",
      "大きい順にならべると 議会の解散 → 首長・議員の解職 → 条例の制定・改廃 → 事務の監査。棒の高さを見比べて確かめる。",
      "有権者60,000人の市の場合。条例・監査は50分の1で1,200人、議会の解散と解職は3分の1で20,000人。身分を失わせる請求だけが桁ちがいに重い（請求先も前二つは首長・監査委員、後二つは選挙管理委員会）",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoshakaiext11_404: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「地方税（自主財源）」で、全体のおよそ33%。",
      "内わけは 地方税（自主財源） 33%、地方交付税交付金 17%、国庫支出金 18%、地方債 10% …。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "地方公共団体の歳入のおおよその内訳（％）。自ら集める地方税は約3分の1にとどまる（三割自治）。地方交付税交付金は使い道が自由、国庫支出金は使い道が指定される。地方債は借金",
    ],
    buildSteps: 4,
