...
export const lessonFigsKokoMathExt11: Record<string, Figure> = {
  ...
  lf_kokomathext11_382: {
    ...
    steps: [
      "よこじくは「時間 t（秒）」、たてじくは「△APD の面積 y（cm²）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（8, 0）。",
  ...
  lf_kokomathext11_383: {
    ...
    steps: [
      ...
      "辺に書かれている長さは BC=6cm、CA=10cm、AB=8cm。どの辺のことかを図で確かめる。",
  ...
  lf_kokomathext11_385: {
    ...
    steps: [
      "よこじくが x、たてじくが y。点の位置は（x, y）の順で読む。",
      "直線は y ＝ 3x。傾きが3で、x が1増えると y は3変わる。",
      "図の点は A(3, 9)、P、Q。",
      "P(t, t²) を通り y 軸に平行な直線が直線 OA（y=3x）と交わる点を Q(t, 3t) とすると PQ=3t−t²。△OAP＝(1/2)×PQ×（A と O の x 座標の差3）＝(3/2)(3t−t²) で、t=3/2 のとき最大値 27/8",
    ],
    buildSteps: 3,
  ...
  lf_kokomathext11_386: {
    ...
    steps: [
      "頂点を順に B・C・A と結んだ三角形。まず形と頂点の名前をおさえる。",
      "辺に書かれている長さは BC=10cm、CA=8cm、AB=6cm。どの辺のことかを図で確かめる。",
  ...
  lf_kokomathext11_388: {
    ...
    steps: [
      "図にある点は A・P・B・M・C・Q・D。どの点がどこにあるかを最初におさえる。",
      "辺に書かれている長さは AP=25/4、PB=15/4、BM=5、MC=5。どの辺のことかを図で確かめる。",
  ...
  lf_kokomathext11_406: {
    ...
    steps: [
      ...
      "この数直線は-6から5まで、1きざみで目もりがふってある。",
      "印がついているのは 解 x=−4、境目、境目、解 x=3。位置を目もりで確かめる。",
      "|x−2|+|x+3|=7 は、絶対値の中身の符号が変わる x=−3 と x=2 で数直線を3つに分けて解く。x<−3 では −2x−1=7 → x=−4（範囲に合う）、−3≤x<2 では 5=7 となり解なし、2≤x では 2x+1=7 → x=3（範囲に合う）",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokomathext11_407: {
    ...
    steps: [
      "よこじくが x、たてじくが y。点の位置は（x, y）の順で読む。",
      "直線は y ＝ 20x ＋ 2000。傾きが20で、x が1増えると y は20変わる。",
      "図の点は (100, 4000)。",
      "A社は基本料2000円＋1分20円、B社は基本料3000円＋1分10円。2直線の交点は (100, 4000) で、通話100分のとき料金が等しくなる。100分より短ければA社、100分より長ければB社が安い",
    ],
    buildSteps: 3,
