...
export const lessonFigsEigoExt11: Record<string, Figure> = {
  ...
  lf_eigoext11_389: {
    ...
    steps: [
      ...
      "印がついているのは S 主語（だれが）、V 動詞（どうする）、O 目的語（何を）、M 修飾語（どこで・いつ）。位置を目もりで確かめる。",
      "英語の語順は原則この順に固定されている。日本語は動詞が文の最後に来るが、英語は主語のすぐ後ろに動詞が来る。だから長文では「2番目のかたまり＝動詞」を探すと骨組みが一気に見える",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext11_394: {
    ...
    steps: [
      ...
      "印がついているのは ①a new bike、②his father、③ it ←、④ he。位置を目もりで確かめる。",
      "文の番号を横軸にとって、代名詞と、それが指す名詞の位置を示した図。代名詞は「自分より前」にある名詞しか指さない。しかもふつうは直前の文の中にある。ただし数（単数・複数）と種類（人か物か）が合う名詞まで戻ること",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext11_397: {
    ...
    steps: [
      ...
      "印がついているのは 一般に言われること、However,、★筆者の主張、まとめ。位置を目もりで確かめる。",
      "説明文の典型的な流れ。「多くの人は〜と思っている」→ However（しかし）→「★でも本当は〜だ」と続く。設問で問われるのは★の部分なので、However・But・Yet を見つけたらその直後に線を引く",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext11_403: {
    ...
    steps: [
      ...
      "いちばん大きいのは「段落の最初」で、全体のおよそ65%。",
      "内わけは 段落の最初 65%、段落の最後 25%、段落の中ほど 10%。",
  ...
  lf_eigoext11_407: {
    ...
    steps: [
      "よこじくは「場面の順序」、たてじくは「気持ち（＋うれしい／−つらい）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（1, 1）、終わりは（5, 3）。",
  ...
  lf_eigoext11_406: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は1から6まで、1きざみで目もりがふってある。",
      "印がついているのは three years ago 引っこし、last spring 入部、in the summer 初ゴール、last winter けが。位置を目もりで確かめる。",
      "出来事を起こった順に並べ直した時間軸。英文では now（現在）から書き始めて、そのあと three years ago まで一気にさかのぼることがある。時を表す語（ago／last／this／now）だけを拾って、この軸の上に並べ直すのがコツ",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext11_409: {
    ...
    steps: [
      "よこじくは「動物」、たてじくは「1日の睡眠時間（時間）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「koala」で20（1日の睡眠時間（時間））。",
      "いちばん小さいのは「giraffe」で2。差は18。",
      "大きい順にならべると koala → cat → human → elephant → giraffe。棒の高さを見比べて確かめる。",
      "本文で比べられている動物の1日の睡眠時間（およその値）。「長く眠る側」と「あまり眠らない側」の2グループに分かれ、その境目に On the other hand（一方で）が置かれている。対比の説明文はこのように2列に整理して読む",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext11_412: {
    ...
    steps: [
      "よこじくは「sport」、たてじくは「students」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「soccer」で11（students）。",
      "いちばん小さいのは「tennis」で2。差は9。",
      "大きい順にならべると soccer → baseball → basketball → swimming → tennis。棒の高さを見比べて確かめる。",
      "「いちばん好きなスポーツ」を30人に聞いた結果。11＋7＋6＋4＋2＝30 で合計がクラスの人数と一致することを必ず確かめる。the most popular＝いちばん高い棒、came second＝2番目、only＝いちばん低い棒、と英語と棒を対応させて読む",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext11_413: {
    ...
    steps: [
      "よこじくは「月」、たてじくは「利用した生徒の数」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（4, 120）、終わりは（9, 180）。",
  ...
  lf_eigoext11_414: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「walk 45%」で、全体のおよそ45%。",
      "内わけは walk 45% 45%、bicycle 30% 30%、bus 15% 15%、train 10% 10%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "200人に聞いた通学手段の割合。円グラフの読解では「％ を人数に直せるか」が必ず問われる。全体200人なので、10％なら200×0.1＝20人。walk と bicycle を合わせると45＋30＝75％である",
    ],
    buildSteps: 4,
  ...
  lf_eigoext11_415: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は8から12まで、1きざみで目もりがふってある。",
      "印がついているのは 8:30 発、9:30 発、10:30 発、11:00 待ち合わせ。位置を目もりで確かめる。",
      "電車の発車時刻（8:30／9:30／10:30）と、博物館前の待ち合わせ時刻（11:00）を1本の時間軸に並べた図。乗車40分＋徒歩10分＝50分かかるので、10:30発では11:20着で間に合わない。時刻表の問題は必ずこの軸を書いてから考える",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext11_417: {
    ...
    steps: [
      "よこじくは「item」、たてじくは「price（円）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「bag」で2500（price（円））。",
      "いちばん小さいのは「socks」で300。差は2200。",
      "大きい順にならべると bag → T-shirt → cap → socks。棒の高さを見比べて確かめる。",
      "買い物の会話に付いている値段表。会話文では「何を・いくつ買ったか」を先に丸で囲み、そのあとで合計を計算する。Tシャツ1枚＋くつ下2足なら 1200＋300×2＝1800円、2000円出せばおつりは200円",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext11_419: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は10から22まで、2きざみで目もりがふってある。",
      "色のついた範囲は 11以上 かつ 14以下。●はその数をふくみ、○はふくまない。",
      "印がついているのは 11:00 ランチ開始、14:00 ランチ終了、17:00 ディナー開始、21:00 閉店。位置を目もりで確かめる。",
      "レストランの営業時間。太い線の部分だけが開いている時間で、14:00〜17:00 は閉まっている。会話文の設問は、この「すきま」をねらって「3時に行ったらどうなるか」「何時なら夕食を食べられるか」と聞いてくる",
    ],
    buildSteps: 4,
  },
