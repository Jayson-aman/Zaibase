...
export const lessonFigsKokoEigoExt12: Record<string, Figure> = {
  ...
  lf_kokoeigoext12_416: {
    ...
    steps: [
      ...
      "いちばん大きいのは「4 長文」で28（配点（点））。",
      "いちばん小さいのは「5 英作文」で12。差は16。",
      "大きい順にならべると 4 長文 → 1 リスニング → 3 会話文 → 2 語形・適語 → 5 英作文。棒の高さを見比べて確かめる。",
      "公立入試の標準的な配点モデル（100点満点）。24＋16＋20＋28＋12＝100点。読解系（大問3・4）だけで48点、リスニングと合わせると72点を占める",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_417: {
    ...
    steps: [
      "よこじくは「作業」、たてじくは「時間（分）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「大問4」で13（時間（分））。",
      "いちばん小さいのは「大問2」で5。差は8。",
      "大きい順にならべると 大問4 → リスニング → 大問3 → 大問5 → 大問2 → 見直し。棒の高さを見比べて確かめる。",
      "試験時間50分の配分モデル。12＋5＋8＋13＋7＋5＝50分。リスニングは放送時間で固定なので、実際に自分で調整できるのは残りの38分である",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_419: {
    ...
    steps: [
      ...
      "いちばん大きいのは「長文の内容一致 10点」で、全体のおよそ33.3%。",
      "内わけは 長文の内容一致 10点 33.3%、英作文 8点 26.7%、文法・語形変化 5点 16.7%、リスニング 4点 13.3% …。",
  ...
  lf_kokoeigoext12_421: {
    ...
    steps: [
      "よこじくは「大問」、たてじくは「配点（点）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「長文A」で30（配点（点））。",
      "いちばん小さいのは「和文英訳」で8。差は22。",
      "大きい順にならべると 長文A → 長文B → 文法・語法 → 整序英作文 → 語彙・熟語 → 和文英訳。棒の高さを見比べて確かめる。",
      "難関私立の配点モデル（100点満点）。30＋25＋15＋10＋12＋8＝100点。長文2題で55点と、公立（28点）の約2倍を占めるのが最大の違い",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_423: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「中学教科書内 60%」で、全体のおよそ60%。",
      "内わけは 中学教科書内 60% 60%、高校基礎レベル 25% 25%、文脈推測前提の難語 10% 10%、注釈付き固有名詞 5% 5%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "難関私立の長文に現れる語のレベル別割合。60＋25＋10＋5＝100%。教科書内の語が6割を占めるので、まず教科書語彙を完成させることが最短ルートになる",
    ],
    buildSteps: 4,
  ...
  lf_kokoeigoext12_425: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「適語選択 35%」で、全体のおよそ35%。",
      "内わけは 適語選択 35% 35%、語形変化 25% 25%、整序英作文 25% 25%、誤文訂正 15% 15%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "公立・私立をならした文法問題の出題形式の割合。35＋25＋25＋15＝100%。形式ごとに解く手順が違うので、形式別に練習すると効率がよい",
    ],
    buildSteps: 4,
  ...
  lf_kokoeigoext12_427: {
    ...
    steps: [
      "よこじくは「出題項目」、たてじくは「正答率（%）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「三単現」で82（正答率（%））。",
      "いちばん小さいのは「現在分詞の後置」で48。差は34。",
      "大きい順にならべると 三単現 → 比較級 → 過去分詞 → 動名詞 → 現在分詞の後置。棒の高さを見比べて確かめる。",
      "語形変化問題の項目別正答率（中3模試の目安）。三単現は8割が正解できるが、現在分詞の後置修飾は5割を切る。落ちている項目から順に埋めるのが得点効率がよい",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_430: {
    ...
    steps: [
      "よこじくは「出題レベル」、たてじくは「語数（語）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「難関私立」で1100（語数（語））。",
      "いちばん小さいのは「公立 会話文」で300。差は800。",
      "大きい順にならべると 難関私立 → 自校作成校 → 公立 長文 → 公立 会話文。棒の高さを見比べて確かめる。",
      "出題レベル別の長文語数の目安。毎分100語で読めれば、公立の長文500語は5分、難関私立の1100語は11分で通読できる計算になる",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_432: {
    ...
    steps: [
      "よこじくは「根拠のある場所」、たてじくは「設問数（問）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「第2段落」で2（設問数（問））。",
      "いちばん小さいのは「第1段落」で1。差は1。",
      "大きい順にならべると 第2段落 → 第3段落 → 全体（内容一致） → 第1段落 → 第4段落。棒の高さを見比べて確かめる。",
      "5段落・設問8問の長文で、各設問の根拠がある場所の分布。1＋2＋2＋1＋2＝8問。設問6問は特定の段落に根拠があり、その段落だけ精読すれば答えが出る",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_434: {
    ...
    steps: [
      "よこじくは「作業」、たてじくは「時間（分）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「本文の通読」で6（時間（分））。",
      "いちばん小さいのは「設問の先読み」で1。差は5。",
      "大きい順にならべると 本文の通読 → 設問の処理 → 設問の先読み → 内容一致の確認。棒の高さを見比べて確かめる。",
      "大問4（長文28点）に配分した13分の内訳。1＋6＋5＋1＝13分。通読に6分しか使えないので、知らない語で止まらないことが時間を守る鍵になる",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_436: {
    ...
    steps: [
      "よこじくは「採点の観点」、たてじくは「配点（点）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「内容（条件充足）」で5（配点（点））。",
      "いちばん小さいのは「つづり・語法」で1。差は4。",
      "大きい順にならべると 内容（条件充足） → 構成（理由・例） → 文法の正確さ → つづり・語法。棒の高さを見比べて確かめる。",
      "英作文12点の採点内訳モデル。5＋3＋3＋1＝12点。文法が完璧でも内容が条件を外せば5点を失う。逆に多少の文法ミスがあっても、条件と構成を守れば8点は残る",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_438: {
    ...
    steps: [
      ...
      "辺に書かれている長さは ①場面の確認（誰に・何のために）、②中心の1文（依頼・提案・感想）、③理由や補足を1〜2文。どの辺のことかを図で確かめる。",
  ...
  lf_kokoeigoext12_440: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「動詞の形 32問」で、全体のおよそ32%。",
      "内わけは 動詞の形 32問 32%、前置詞の過不足 24問 24%、冠詞・名詞の数 20問 20%、語順 14問 14% …。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "誤文訂正100問の誤りの型の内訳。32＋24＋20＋14＋10＝100問。動詞の形と前置詞だけで56問、半分以上を占める",
    ],
    buildSteps: 4,
  ...
  lf_kokoeigoext12_443: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「単数可算名詞に a/an がない 18」で、全体のおよそ36%。",
      "内わけは 単数可算名詞に a/an がない 18 36%、複数形の -s 落ち 15 30%、不可算名詞に a/-s を付けた 9 18%、the の要不要 8 16%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "冠詞・名詞の数の誤り50個の内訳。18＋15＋9＋8＝50個。「a を付け忘れる」「-s を落とす」の2つで33個、3分の2を占める",
    ],
    buildSteps: 4,
  ...
  lf_kokoeigoext12_444: {
    ...
    steps: [
      "よこじくは「点検項目」、たてじくは「平均回復点（点）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「名詞の複数形 -s」で2（平均回復点（点））。",
      "いちばん小さいのは「つづり」で1。差は1。",
      "大きい順にならべると 名詞の複数形 -s → 三単現の -s → 空欄・記号もれ → つづり → 冠詞の抜け。棒の高さを見比べて確かめる。",
      "最後の5分の見直しで平均的に回復できる点数。2＋2＋1＋1＋2＝8点。新しい問題を1問解くより、この8点を取り戻すほうが確実性が高い",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokoeigoext12_448: {
    ...
    steps: [
      "よこじくは「学習内容」、たてじくは「1日あたりの時間（分）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「過去問演習」で40（1日あたりの時間（分））。",
      "いちばん小さいのは「英作文の型の音読」で5。差は35。",
      "大きい順にならべると 過去問演習 → 誤答ノート → 単語・熟語 → リスニング → 英作文の型の音読。棒の高さを見比べて確かめる。",
      "直前14日間の1日90分の配分モデル。40＋20＋15＋10＋5＝90分。新しい教材ではなく、すでに解いた問題の復習に60分（過去問＋誤答ノート）を充てるのが直前期の原則",
    ],
    buildSteps: 4,
  },
