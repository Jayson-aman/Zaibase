...
export const lessonFigsKokoRikaExt05: Record<string, Figure> = {
  ...
  lf_kokorikaext05_146: {
    ...
    steps: [
      "よこじくは「時間 (s)」、たてじくは「移動距離 (m)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（40, 80）。",
  ...
  lf_kokorikaext05_148: {
    ...
    steps: [
      "よこじくは「5打点ごとに切ったテープ（東日本・1区間0.1秒）」、たてじくは「テープの長さ (cm)」を表している。まず何と何を比べた図かをつかむ。",
      "どの棒も同じ高さで、値はすべて3（テープの長さ (cm)）。",
  ...
  lf_kokorikaext05_149: {
    ...
    steps: [
      "よこじくは「5打点ごとに切ったテープ（東日本・1区間0.1秒）」、たてじくは「テープの長さ (cm)」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「⑥」で11（テープの長さ (cm)）。",
      "いちばん小さいのは「①」で1。差は10。",
      "大きい順にならべると ⑥ → ⑤ → ④ → ③ → ② → ①。棒の高さを見比べて確かめる。",
      "静止から動き出した台車のテープ（東日本50Hz・5打点＝0.1秒ごとに切断）。長さは1.0→3.0→5.0→7.0→9.0→11.0cmで、増え方が2.0cmずつ一定。各区間の平均の速さは0.10／0.30／0.50／0.70／0.90／1.10m/sとなり、0.1秒あたり0.20m/sずつ増える。0.6秒間の移動距離は合計36cm＝0.36m",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokorikaext05_150: {
    ...
    steps: [
      "よこじくは「時間 (s)」、たてじくは「速さ (m/s)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0.05, 0.1）、終わりは（0.55, 1.1）。",
  ...
  lf_kokorikaext05_158: {
    ...
    steps: [
      ...
      "辺に書かれている長さは 底辺 4.0 m、高さ 3.0 m、斜面 5.0 m。どの辺のことかを図で確かめる。",
  ...
  lf_kokorikaext05_159: {
    ...
    steps: [
      "よこじくは「落ち始めてからの時間 (s)」、たてじくは「速さ (m/s)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（2, 19.6）。",
      "全体として19.6だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
      "かたむきが急なところほど、変わり方が大きい。",
      "空気の抵抗を無視した自由落下の速さ－時間グラフ。1秒ごとに9.8m/sずつ速くなるので、原点を通る傾き9.8の直線になる（v＝9.8×t）。落ちた距離はグラフと横軸で囲む三角形の面積で、2秒後なら½×2×19.6＝19.6m（h＝4.9×t²でも同じ）。この直線は物体の質量を変えても変わらない",
    ],
    buildSteps: 4,
  ...
  lf_kokorikaext05_171: {
    ...
    steps: [
      "よこじくは「40Nの荷物を高さ1.5mまで上げる方法」、たてじくは「必要な力 (N)」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「直接」で40（必要な力 (N)）。",
      "いちばん小さいのは「動滑車2個」で10。差は30。",
      "大きい順にならべると 直接 → 動滑車1個 → 斜面3.0m → 動滑車2個。棒の高さを見比べて確かめる。",
      "重さ40Nの荷物を高さ1.5mまで上げるのに必要な力。直接なら40Nで1.5m引き、動滑車1個なら20Nで3.0m、動滑車2個なら10Nで6.0m、長さ3.0mの斜面なら20Nで3.0m引く。力は小さくなるが引く距離がそのぶん長くなるので、仕事はどれも60J（＝40×1.5＝20×3.0＝10×6.0）で変わらない",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokorikaext05_172: {
    ...
    steps: [
      "頂点を順に A・B・C と結んだ三角形。まず形と頂点の名前をおさえる。",
      "辺に書かれている長さは 底辺 約3.5 m、高さ 2.0 m、斜面 4.0 m。どの辺のことかを図で確かめる。",
      "B のところが直角。直角があれば三平方の定理や面積の式が使える。",
      "高さ2.0m・斜面の長さ4.0mのなめらかな斜面。重さ60Nの荷物を斜面にそって引き上げるとき、必要な力は60N×(2.0÷4.0)＝30Nで半分になるが、引く距離は4.0mと2倍になる。仕事は30N×4.0m＝120Jで、直接持ち上げる60N×2.0m＝120Jと等しい",
    ],
    buildSteps: 3,
  ...
  lf_kokorikaext05_176: {
    ...
    steps: [
      "よこじくは「基準面からの高さ (m)」、たてじくは「位置エネルギー (J)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（3, 30）。",
  ...
  lf_kokorikaext05_177: {
    ...
    steps: [
      "よこじくは「速さ (m/s)」、たてじくは「運動エネルギー (J)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（4, 16）。",
  ...
  lf_kokorikaext05_178: {
    ...
    steps: [
      "よこじくは「おもりを落とす高さ (cm)」、たてじくは「杭が打ちこまれた深さ (cm)」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「40」で2（杭が打ちこまれた深さ (cm)）。",
      "いちばん小さいのは「10」で0.5。差は1.5。",
      "大きい順にならべると 40 → 30 → 20 → 10。棒の高さを見比べて確かめる。",
      "同じおもりを高さを変えて落とし、杭の打ちこまれた深さを測った結果。深さは高さに比例している（10cmで0.5cm、40cmで2.0cm＝4倍）。杭を打つ仕事のもとは落とす前の位置エネルギー＝重さ×高さなので、深さは「高さ」に比例する。ぶつかる直前の速さは高さの平方根に比例するので、深さは速さには比例しないことに注意",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokorikaext05_181: {
    ...
    steps: [
      "よこじくは「時間（1往復を1とした割合）」、たてじくは「エネルギー (J)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 1）、終わりは（1, 1）。",
      "はじめと終わりが同じ値。とちゅうの動きに注目する。",
  ...
  lf_kokorikaext05_182: {
    ...
    steps: [
      "よこじくが x、たてじくが y。点の位置は（x, y）の順で読む。",
      "放物線は y ＝ 0.2x²。原点を頂点とし、y じくについて左右対称になる。",
      "図の点は A 高さ5.0m、B 高さ0m、C 高さ1.8m。",
      "なめらかな谷型のコース（横軸は水平方向の位置m、縦軸は高さm）を、質量1.0kg（重さ10N）の球がA点から静かに転がり落ちるようす。A（高さ5.0m）の位置エネルギーは10N×5.0m＝50J。B（高さ0m）では位置エネルギー0・運動エネルギー50Jとなり、½×1.0×v²＝50よりv＝10m/s。C（高さ1.8m）では位置エネルギー18J・運動エネルギー32Jとなりv＝8.0m/s。合計はどこでも50Jで一定",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokorikaext05_185: {
    ...
    steps: [
      ...
      "いちばん大きいのは「排熱として放出 60%」で、全体のおよそ60%。",
      "内わけは 電気エネルギー 40% 40%、排熱として放出 60% 60%。",
