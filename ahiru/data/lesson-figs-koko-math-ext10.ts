...
export const lessonFigsKokoMathExt10: Record<string, Figure> = {
  ...
  lf_kokomathext10_349: {
    ...
    steps: [
      "よこじくは「2つのさいころの目の和」、たてじくは「場合の数（通り）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「7」で6。ここが最頻値をふくむ階級になる。",
      "度数の合計は36。全体の中でどのあたりに集まっているかを見る。",
      "和は2〜12の11種類だが、起こりやすさは同じではない。合計は36通りで、和7が6通り（＝6/36＝1/6）と最も多い。「11通りだから1/11」としてはいけない理由がこの形に表れている",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokomathext10_350: {
    ...
    steps: [
      "よこじくは「表が出た枚数」、たてじくは「場合の数（通り）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「1枚」で3。ここが最頻値をふくむ階級になる。",
      "度数の合計は8。全体の中でどのあたりに集まっているかを見る。",
      "硬貨3枚の出方は 2×2×2＝8通り。表の枚数は0〜3の4種類だが、1枚・2枚が各3通りで起こりやすい。「4種類だから1/4」は誤り（表2枚は 3/8）",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokomathext10_351: {
    ...
    steps: [
      "よこじくは「2つのさいころの目の差」、たてじくは「場合の数（通り）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「1」で10。ここが最頻値をふくむ階級になる。",
      "度数の合計は36。全体の中でどのあたりに集まっているかを見る。",
      "目の差は0〜5の6種類だが度数はばらばら（合計36通り）。差1が10通りで最多、差5は(1,6)(6,1)の2通りだけ。差2は8通りなので確率は 8/36＝2/9",
    ],
    buildSteps: 4,
  ...
  lf_kokomathext10_356: {
    ...
    steps: [
      "よこじくは「取り出し方（赤3個・白2個の袋から2個）」、たてじくは「確率（％）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「赤白:もどさない」で60（確率（％））。",
      "いちばん小さいのは「白白:もどさない」で10。差は50。",
      "大きい順にならべると 赤白:もどさない → 赤白:もどす → 赤赤:もどす → 赤赤:もどさない → 白白:もどす → 白白:もどさない。棒の高さを見比べて確かめる。",
      "同じ袋でも「もどす」と「もどさない」で確率は変わる。同じ色がそろう確率は下がり（36→30、16→10）、ちがう色になる確率は上がる（48→60）。どちらも3つの合計はちょうど100％",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokomathext10_357: {
    ...
    steps: [
      "よこじくは「さいころを投げる回数」、たてじくは「少なくとも1回1が出る確率（％）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（6, 66.5）。",
      "全体として66.5だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
  ...
  lf_kokomathext10_358: {
    ...
    steps: [
      ...
      "いちばん大きいのは「少なくとも1本当たり 24通り」で、全体のおよそ53.3%。",
      "内わけは 少なくとも1本当たり 24通り 53.3%、2本ともはずれ 21通り 46.7%。",
  ...
  lf_kokomathext10_360: {
    ...
    steps: [
      "よこじくは「記録（m）」、たてじくは「度数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「20〜25」で14。ここが最頻値をふくむ階級になる。",
      "度数の合計は50。全体の中でどのあたりに集まっているかを見る。",
      "50人のハンドボール投げ。階級の幅は5m、度数の合計は50人。最頻値は度数最大の階級の階級値22.5m、平均値は（階級値×度数）の合計1245÷50＝24.9m。25m以上は 12+8+4＝24人",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokomathext10_361: {
    ...
    steps: [
      "よこじくは「通学時間（分）」、たてじくは「度数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「10〜20」で15。ここが最頻値をふくむ階級になる。",
      "度数の合計は50。全体の中でどのあたりに集まっているかを見る。",
      "右に長い裾を引く分布。中央値（25番目と26番目）は10〜20分の階級に入るが、遠くから通う少数の人に引っぱられて平均値は1130÷50＝22.6分になる。裾を引く向きの側へ平均値がずれる",
    ],
    buildSteps: 4,
  ...
  lf_kokomathext10_362: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「20〜40点 16人(0.32)」で、全体のおよそ32%。",
      "内わけは 0〜20点 3人(0.06) 6%、20〜40点 16人(0.32) 32%、40〜60点 12人(0.24) 24%、60〜80点 12人(0.24) 24% …。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "相対度数は「全体を1とみたときの割合」＝度数÷総度数。50人なので 3/50＝0.06、16/50＝0.32、12/50＝0.24、7/50＝0.14。相対度数の合計は必ず1.00になる（検算に使える）",
    ],
    buildSteps: 4,
  ...
  lf_kokomathext10_363: {
    ...
    steps: [
      "よこじくは「得点（点）」、たてじくは「累積相対度数（％）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（100, 100）。",
  ...
  lf_kokomathext10_364: {
    ...
    steps: [
      "よこじくは「1か月に読んだ本の冊数」、たてじくは「人数（人）」を表している。まず何と何を比べた図かをつかむ。",
      "階級ごとの個数を表したヒストグラム。棒がとなり合っているのは、階級が連続しているため。",
      "いちばん度数が大きい階級は「2」で8。ここが最頻値をふくむ階級になる。",
  ...
  lf_kokomathext10_366: {
    ...
    steps: [
      "よこじくは「代表値のくらべ方」、たてじくは「点数（点）」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「平均:外れ値なし」で70（点数（点））。",
      "いちばん小さいのは「平均:0点が1人」で58.3。差は11.7。",
      "大きい順にならべると 平均:外れ値なし → 中央値:外れ値なし → 中央値:0点が1人 → 平均:0点が1人。棒の高さを見比べて確かめる。",
      "60・65・70・75・80点の5人（平均70点、中央値70点）に0点の1人が加わると、平均は 350÷6＝58.3点へ11.7点も下がるのに、中央値は (65+70)÷2＝67.5点で2.5点しか動かない。外れ値があるときは中央値の方が実態に合う",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokomathext10_374: {
    ...
    steps: [
      "よこじくは「年」、たてじくは「売上高（億円）」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（2021, 100）、終わりは（2025, 108）。",
