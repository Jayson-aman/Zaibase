...
export const lessonFigsEigoExt03: Record<string, Figure> = {
  ...
  lf_eigoext03_081: {
    ...
    steps: [
      ...
      "いちばん大きいのは「可算のみ」で、全体のおよそ70%。",
      "内わけは 可算のみ 70%、不可算のみ 15%、両方ある 15%。",
  ...
  lf_eigoext03_089: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「-s をつける」で、全体のおよそ80%。",
      "内わけは -s をつける 80%、-es をつける 10%、y→ies 6%、不規則 4%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "名詞の複数形の作り方のおおよその割合。ほとんどは -s を付けるだけで済む。不規則変化は man / child / foot / tooth / mouse など十数語しかないので、そこだけ丸暗記すれば足りる",
    ],
    buildSteps: 4,
  ...
  lf_eigoext03_090: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「[z] dogs 型」で、全体のおよそ60%。",
      "内わけは [z] dogs 型 60%、[s] cats 型 25%、[iz] buses 型 15%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "複数形の -s の発音のおおよその割合。のどがふるえる音（有声音）のあとは [z]、ふるえない音（無声音）のあとは [s]、s・z・sh・ch・x のあとは [iz] と読み、音のかたまりが一つふえる",
    ],
    buildSteps: 4,
  ...
  lf_eigoext03_094: {
    ...
    steps: [
      ...
      "いちばん大きいのは「three glasses of milk」で3（コップの数）。",
      "いちばん小さいのは「a glass of milk」で1。差は2。",
      "大きい順にならべると three glasses of milk → two glasses of milk → a glass of milk。棒の高さを見比べて確かめる。",
      "棒の高さはコップの数。数がふえても milk は milks にならず、数えるための単位のことば glass のほうが glasses に変わる。ここが「単位＋of＋不可算名詞」の一番の急所",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext03_098: {
    ...
    steps: [
      "棒の高さが数の大きさを表している。まず何を比べた図かをつかむ。",
      "いちばん大きいのは「every day」で7（1週間あたりの回数）。",
      "いちばん小さいのは「once a week」で1。差は6。",
      "大きい順にならべると every day → three times a week → twice a week → once a week。棒の高さを見比べて確かめる。",
      "once a week などの a は「一つの」ではなく「〜につき」の意味。回数は once（1回）→ twice（2回）→ three times（3回）と数え、3回以上は「数字＋times」で表す",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext03_099: {
    ...
    steps: [
      ...
      "印がついているのは a dog（初めて話に出す）、the dog（さっきのあの犬）。位置を目もりで確かめる。",
      "話の進む順に並べた図。I saw a dog. The dog was very big. のように、初めて出すときは a、いちど出て「あの犬」と相手にも特定できるようになったら the を使う",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext03_105: {
    ...
    steps: [
      ...
      "印がついているのは She（主格）、likes、him（目的格）。位置を目もりで確かめる。",
      "She likes him. の語順。英語は位置で役割が決まるので、動詞の前（主語の位置）なら主格 She、動詞のうしろ（目的語の位置）なら目的格 him になる。日本語の「彼女は／彼を」にあたる区別を、英語は語の形そのもので示す",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext03_112: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は0から4まで、1きざみで目もりがふってある。",
      "印がついているのは one（1つ目）、another（残りのうちもう1つ）、the other（最後の1つ）。位置を目もりで確かめる。",
      "3つのものを順に取り上げるときの図。残りがまだ複数あるうちは another（an + other なので冠詞は付けない）、最後の1つに決まったら the other。2つしかない場合は one と the other だけになる",
    ],
    buildSteps: 3,
  },
