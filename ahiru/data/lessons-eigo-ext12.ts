...
export const eigoExt12Lessons: Lesson[] = [
  // ───────────────── 1. 和文英訳の手順（s426〜s428） ─────────────────
  {
    ...
    intro: '和文英訳でいきなり日本語の頭から訳し始めると、たいてい途中で行きづまります。日本語は動詞が最後、英語は動詞が二番目という順番のちがいがあるからです。訳すのではなく、英語の型に置きかえる手順を身につけます。',
    order: 926,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：一つの文に動詞が二つ並んだら、その時点でほぼ誤りである。書き終えたら「動詞は一つか」を必ず確かめる。`,
...

  {
    ...
    intro: '「毎朝六時に起きます」と「今起きたところです」では、使う動詞の形がちがいます。手がかりは日本語の文末にあります。〜します、〜しています、〜しました——文末を見るだけで時制はほぼ決まります。その対応を整理します。',
    order: 927,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：Yesterday was rain. と書く答案が非常に多い。日本語の「きのうは」を主語だと思ってしまうためだが、yesterday は時を表す語であって主語にはならない。`,
...

  {
    ...
    intro: '「祖父は畑を耕しています」を英語にしようとして、「耕す」が分からず手が止まる——よくある場面です。ここで必要なのは辞書ではなく、日本語を作り直す力です。「野菜を作っています」なら書けます。言いかえの技術を学びます。',
    order: 928,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：③の前置詞のあとがいちばんまちがえやすい。at, in, of, about, before, after はすべて前置詞なので、そのあとに動詞を置くなら必ず ing 形にする。He is good at play the piano. は誤りである。`,
  ...
  // ───────────────── 2. 主語の扱い（s429〜s430） ─────────────────
  {
    ...
    intro: '「昨日、公園へ行きました」——この日本語には主語がありません。それでも日本語では通じます。ところが英語は、主語がないと文になりません（命令文だけが例外です）。省かれた主語を見つけて立てる手順を確かめます。',
    order: 929,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：There is my bag ~ のような誤りは、日本語の「〜があります」をすべて There is に置きかえてしまうことから起きる。「初めて出す話題か、もう知っているものか」で使い分ける。`,
  ...
  // ───────────────── 3. 語順並べかえ（s431〜s433） ─────────────────
  {
    ...
    intro: '並べかえ問題で、語群を先頭から順に組み立てていくと迷います。先にすることは動詞さがしです。動詞が決まれば主語が決まり、主語が決まれば目的語も決まります。骨組みを作ってから飾りを入れる——この順番で解きます。',
    order: 931,
    studyPeriod: '小6前半',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：Can you show the way to the station me? のように「人」を最後に置くのは誤り。人を後ろに回すなら to me が必要である。語群に to や for が入っているかどうかで、どちらの型を作るかを判断する。`,
...

  {
    ...
    intro: '語群に a・very・long・book が入っていたら、どう並べるか決められますか。一語の形容詞は名詞の前、二語以上のかたまりは名詞のうしろ、頻度の副詞は一般動詞の前——修飾語の位置には決まりがあります。それを型にします。',
    order: 932,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：目的語（English など）と動詞の間に副詞を割りこませない。She speaks well English. のような答案が多いが、目的語のあとに置いて She speaks English well. とする。`,
...

  {
    ...
    intro: '入試の並べかえで落としやすいのは、間接疑問・不定詞・比較の三つです。とくに間接疑問は、文の中に入ると疑問文の語順ではなくなります。where he is を where is he と書いてしまう——この失点をここでなくします。',
    order: 933,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：good — better — best、bad — worse — worst、many/much — more — most、high — higher — highest は特別。high は短い語なので more high とはしない。`,
  ...
  // ───────────────── 4. 条件英作文（s434〜s436） ─────────────────
  {
    ...
    intro: '条件英作文では、内容が正しくても指定された語を使っていなければ零点になります。逆に言えば、条件を守るだけで確実に点が残ります。語を使う、書き出しに続ける、形を変えてよいか——条件の読み取り方を確かめます。',
    order: 934,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：enjoy の直後に to 不定詞は置けない。I enjoyed to play soccer. は誤りで、I enjoyed playing soccer. とする。enjoy / finish / stop / practice のあとは ing、want / hope / decide / try のあとは to、とセットで覚える。`,
...

  {
    ...
    intro: '「十語以内」と言われたとき、I\'m は一語でしょうか、二語でしょうか。答えは一語です。e-mail のようにハイフンでつないだ語も一語、ice cream のように離して書く語は二語。数え方の決まりを先に固めておきます。',
    order: 935,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：語数が一語でも条件から外れると、内容が正しくても0点になる採点が多い。書き終えたら必ず指で語を押さえながら数える。数えた語数を答案の横に小さくメモしておくと見直しが速い。`,
...

  {
    ...
    intro: '対話文の空所を埋める問題では、空所のうしろの返事がいちばんの手がかりです。答えが時刻なら What time、値段なら How much を入れればよいからです。返事から逆算して質問を作る、この解き方を練習します。',
    order: 936,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：How about ~ ? は Do you ~ ? の形をしていないので、Yes, I do. とは答えられない。Sure. / That sounds good. / Yes, let's. のような応答を使う。答え方の形が質問の形に合っているかを必ず確かめる。`,
  ...
  // ───────────────── 5. 自由英作文（s437〜s439） ─────────────────
  {
    ...
    intro: '自由英作文で「どちらも好きです」と書くと、正直なのに減点されます。立場をはっきり決めることが求められているからです。意見、理由二つ、まとめ——四文の型を持っておけば、白紙のまま悩む時間がなくなります。',
    order: 937,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：理由が一つしかないのに二つに分けて書こうとすると、同じことのくり返しになって内容点が下がる。「楽しいから」と「おもしろいから」は同じ理由である。角度の違う理由を二つ用意する。`,
...

  {
    ...
    intro: '「賛成です。楽しいからです。」——これでは理由が一文で終わり、点が伸びません。そこに具体例か自分の体験を一文足すだけで、答案の厚みは変わります。For example や去年の出来事を、どう足すかを練習します。',
    order: 938,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：覚えた言い回しをそのまま使うときも、主語に合わせて動詞を変える必要がある。It helps me study English. の主語が複数なら They help me study English. になる。丸暗記した文をそのまま貼りつけて三単現をまちがえる答案が非常に多い。`,
...

  {
    ...
    intro: '書き終えた瞬間に手を止めてはいけません。減点の多くは、内容ではなく三単現の s や冠詞といった小さなミスから生まれるからです。見直す順番を決めておけば、三十秒で数点を拾えます。その順序を身につけます。',
    order: 939,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：play the soccer と play piano は、どちらもよくある誤り。「スポーツは冠詞なし、楽器は the」と対にして覚える。`,
  ...
  // ───────────────── 6. 絵・写真の描写（s440〜s442） ─────────────────
  {
    ...
    intro: '絵を見て英文を作る問題では、まず「だれが何をしているか」を書きます。今まさにしている動作なので、現在進行形を使います。ing の付け方でつまずくことが多く、run は running と n を重ねます。書きながら確かめます。',
    order: 940,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：wear は「状態」を表す動詞だが、進行形にできる例外である。「着ている最中」ではなく「着ている状態」を表すので、絵の描写では is wearing がよく使われる。`,
...

  {
    ...
    intro: '机の上のノートは on、引き出しの中は in、頭の上にかかった時計は above。絵の描写では、位置を表す前置詞をまちがえると意味が変わってしまいます。ふれているかいないか、という基準で選び分けます。',
    order: 941,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：「〜の間に」を日本語でひとくくりにして among を使うと誤りになる。二つなら必ず between。between the two trees のように、二つであることを the two ~ と示すと確実である。`,
...

  {
    ...
    intro: '絵に牛乳のコップが三つあっても、three milks とは書けません。milk は数えられない名詞だからです。数えられるものと数えられないもので、量の表し方が変わります。色や様子を足して描写を豊かにする方法も学びます。',
    order: 942,
    studyPeriod: '小6前半',
    targetLevel: 'oyo',
    keyPoints: [
...
⚠ 注意：three childs, two mans のような答案が多い。不規則な複数形は数が限られているので、まとめて覚えてしまう。`,
  ...
  // ───────────────── 7. 誤文訂正（s443〜s445） ─────────────────
  {
    ...
    intro: '誤りをさがす問題では、どこから見ていくかを決めておくと速くなります。いちばん多いのは s のつけ忘れとつけすぎです。主語が三人称単数で現在なら動詞に s、名詞が二つ以上なら名詞に s。まずここを確かめます。',
    order: 943,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：一つの文に s は一回だけ、という覚え方は誤り。My brothers play soccer.（名詞に s、動詞には s なし）と My brother plays soccer.（名詞に s なし、動詞に s）は、s の位置がちょうど入れかわる。主語と動詞をセットで見る。`,
...

  {
    ...
    intro: 'a hour と書くと誤りで、正しくは an hour です。h を発音しないからです。逆に university は u で始まるのに a university。a と an は、つづりではなく最初の音で決まります。冠詞の落とし穴を確かめます。',
    order: 944,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：日本語には冠詞がないので、つけ忘れは意識しないと絶対に減らない。名詞を書いたら、そのたびに「一つか、複数か、数えられないか」を心の中で確かめる習慣をつける。`,
...

  {
    ...
    intro: 'I have finished it yesterday. はどこが誤りでしょうか。現在完了は yesterday のような、過去の一点を表す語と組めません。時を表す語と動詞の形が合っているか、前置詞が抜けていないか。二つの観点で見直します。',
    order: 945,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：get to ~（〜に着く）は前置詞が要るが、get home は要らない。arrive も arrive at the station だが arrive home となる。home が特別だと覚えておく。`,
  ...
  // ───────────────── 8. リスニング（s446〜s447） ─────────────────
  {
    ...
    intro: 'リスニングは、放送が始まってから考えていては間に合いません。勝負は始まる前です。選択肢を先に読み、時刻が並んでいるのか場所なのかを見ておけば、何を待ちかまえて聞けばよいかが決まります。その準備のしかたを学びます。',
    order: 946,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：数字が二つ以上聞こえたら、たし算・かけ算・引き算のどれかを求められている可能性を疑う。メモには数字だけでなく「2本」「1本150円」のように単位も書いておく。`,
...

  {
    ...
    intro: 'thirteen と thirty は、聞き分けられますか。ちがいはアクセントの位置で、thirteen は後ろ、thirty は前が強く読まれます。14と40、15と50も同じ関係です。数字と名前をどうメモするかまで含めて練習します。',
    order: 947,
    studyPeriod: '小6後半・直前',
    targetLevel: 'moshi',
    keyPoints: [
...
⚠ 注意：メモは答えではない。放送が終わったら、メモを見ながら設問の疑問詞（What time / Where / How many）に合う情報を選んで答える。`,
  ...
  // ───────────────── 9. 総合実戦演習（s448〜s450） ─────────────────
  {
    ...
    intro: 'ここまで学んだ語形変化・並べかえ・和文英訳・条件英作文は、実際の入試では一枚の問題用紙に並んで出てきます。一つずつなら解けても、通しで解くと時間が足りなくなるものです。本番と同じ形で力を確かめてみましょう。',
    order: 948,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：第4問で I play soccer.（3語）と書くと、内容は正しくても語数不足で0点になる。答えを書いたら必ず語を数え、条件を満たしているかを確かめてから次へ進む。`,
...

  {
    ...
    intro: '帰国子女入試では、六十語程度の英作文や、本文にもとづく記述が求められます。短い答えを積み上げる入試とは、必要な力がちがいます。書く量と正確さを両立させるには型が要ります。その型を実際の問題で試します。',
    order: 949,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
...
⚠ 注意：英語で答える設問に日本語で答える、日本語で答える設問を英語で答える、という取りちがえは毎年多い。設問文の書かれている言語をまず確かめる。`,
...

  {
    ...
    intro: '同じ実力でも、解く順番と時間配分で点数は変わります。リスニングだけは自分で時間を選べないので、それ以外をどう並べるかが勝負です。英作文と見直しの時間をどう確保するか、当日の段取りをここで決めておきます。',
    order: 950,
    studyPeriod: '小6後半・直前',
    targetLevel: 'nyushi',
    keyPoints: [
