...
export const lessonFigsKokugoExt01: Record<string, Figure> = {
  ...
  lf_kokugoext01_008: {
    ...
    steps: [
      ...
      "にょう（辶・廴・走）が占める位置。左側から始まって下側まで回りこむL字形になる。上だけの「かんむり」、上から左下までの「たれ」との形のちがいに注目する",
    ],
    buildSteps: 2,
  },
  ...
  lf_kokugoext01_009: {
    ...
    steps: [
      ...
      "いちばん大きいのは「形声」で、全体のおよそ80%。",
      "内わけは 形声 80%、会意 12%、象形 6%、指事 2%。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "漢字の成り立ちのおおよその割合。形声文字が約8割を占めるので、知らない漢字の読みは音を表す部分（つくり）から推測できることが多い",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokugoext01_012: {
    ...
    steps: [
      ...
      "「対称」は左右がつりあった形のこと（算数の線対称と同じ）。「対象＝はたらきかける相手」「対照＝比べてちがいが際立つこと」とは別の語",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokugoext01_027: {
    ...
    steps: [
      ...
      "この数直線は1から20まで、1きざみで目もりがふってある。",
      "印がついているのは ついたち、よっか、ようか、じゅうよっか。位置を目もりで確かめる。",
      "日付の特別な読み。とくに「四日（よっか）」と「八日（ようか）」、「二十日（はつか）」はまちがえやすい",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokugoext01_028: {
    ...
    steps: [
      ...
      "印がついているのは 睦月、弥生、水無月、長月。位置を目もりで確かめる。",
      "旧暦の月の異名（代表例）。旧暦は今の暦よりおよそ一か月おそいので、旧暦五月の「五月雨」は今の六月ごろの長雨（梅雨）を指す",
    ],
    buildSteps: 3,
  },
  ...
  lf_kokugoext01_035: {
    ...
    steps: [
      ...
      "いちばん大きいのは「無」で12（画数）。",
      "いちばん小さいのは「必」で5。差は7。",
      "大きい順にならべると 無 → 飛 → 成 → 必 → 右。棒の高さを見比べて確かめる。",
      "数えまちがえやすい漢字の総画数。「無」の下の灬（れっか）は4画、しんにょう・くさかんむりは3画で数える",
    ],
    buildSteps: 4,
  },
  ...
  lf_kokugoext01_039: {
    ...
    steps: [
      "よこじくは「学年」、たてじくは「字数」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「4年」で202（字数）。",
      "いちばん小さいのは「1年」で80。差は122。",
      "大きい順にならべると 4年 → 3年 → 5年 → 6年 → 2年 → 1年。棒の高さを見比べて確かめる。",
      "学年別漢字配当表の字数。合計1026字で、これが中学入試の書き取りの中心範囲（常用漢字は全部で2136字）",
    ],
    buildSteps: 4,
  },
};
