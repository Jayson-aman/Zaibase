...
export const lessonFigsRikaExt07: Record<string, Figure> = {
  ...
  r7f_s215: {
    ...
    steps: [
      "よこじくは「電圧(V)」、たてじくは「電流(A)」。点をつないだ線が、どう変わっていくかを表している。",
      "はじめは（0, 0）、終わりは（4, 1）。",
      "全体として1だけ増えている。右上がりなら増加、右下がりなら減少と読む。",
      "線が2本あるので、同じよこじくの値どうしで上下を見比べる。",
      "R＝V÷I。R₂は電圧4V・電流0.5Aの点を通るのでR＝4÷0.5＝8Ω。傾きが緩やかなほど抵抗が大きい",
    ],
    buildSteps: 4,
  },
  ...
  r7f_s216: {
    ...
    steps: [
      ...
      "いちばん大きいのは「元の導線(長さL・断面積A)」で8（抵抗(Ω)）。",
      "いちばん小さいのは「長さ1/2倍・断面積2倍」で2。差は6。",
      "2本を見比べて、どちらがどれだけ大きいかをつかむ。",
      "抵抗は長さに比例し断面積に反比例。8Ω×(1/2)×(1/2)＝2Ω。4Vをかけると電流は4÷2＝2A",
    ],
    buildSteps: 4,
  },
  ...
  r7f_s221: {
    ...
    steps: [
      "棒の高さが数の大きさを表している。まず何を比べた図かをつかむ。",
      "いちばん大きいのは「抵抗1個(10Ω)」で2（電流(A)）。",
      "いちばん小さいのは「抵抗2個を直列で追加(20Ω)」で1。差は1。",
      "2本を見比べて、どちらがどれだけ大きいかをつかむ。",
      "V＝20V一定。合成抵抗が10Ω→20Ωに増えると、電流は2A→1Aに減る",
    ],
    buildSteps: 4,
  },
  ...
  r7f_s222: {
    ...
    steps: [
      "棒の高さが数の大きさを表している。まず何を比べた図かをつかむ。",
      "いちばん大きいのは「豆電球1個(R5Ω)」で2（電流(A)）。",
      "いちばん小さいのは「豆電球2個を直列(R10Ω)」で1。差は1。",
      "2本を見比べて、どちらがどれだけ大きいかをつかむ。",
      "V＝10V一定。電球を直列で増やすと合成抵抗が増え、電流が減って暗くなる",
    ],
    buildSteps: 4,
  },
  ...
  r7f_s228: {
    ...
    steps: [
      "棒の高さが数の大きさを表している。まず何を比べた図かをつかむ。",
      "いちばん大きいのは「電球を流れる電流(電池1個でも2個並列でも)」で0.5（電流(A)）。",
      "いちばん小さいのは「電池1個あたりが受け持つ電流(2個並列時)」で0.25。差は0.25。",
      "2本を見比べて、どちらがどれだけ大きいかをつかむ。",
      "電池を並列にしても電圧は1.5Vのまま。電球の電流0.5Aは変わらず、電池2個で半分ずつ(0.25A)分担する",
    ],
    buildSteps: 4,
  },
