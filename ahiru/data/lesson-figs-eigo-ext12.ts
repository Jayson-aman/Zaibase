...
export const lessonFigsEigoExt12: Record<string, Figure> = {
  ...
  lf_eigoext12_427: {
    ...
    steps: [
      ...
      "印がついているのは 〜した→過去形、今〜している→be＋ing、〜するつもり→will。位置を目もりで確かめる。",
      "日本語の文末を見て、時間軸のどこに置くかを決めてから動詞の形を選ぶ。「毎日〜します」のように習慣を表す文だけは時間軸の一点ではなく全体を指すので現在形にする",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext12_431: {
    ...
    steps: [
      ...
      "印がついているのは だれが、どうする、何を、どこで。位置を目もりで確かめる。",
      "並べかえ問題の語順のものさし。I played soccer in the park yesterday. のように、文の後ろは「場所→時」の順に置く。日本語の「きのう公園で」とは逆になる点に注意する",
    ],
    buildSteps: 3,
  },
  ...
  lf_eigoext12_435: {
    ...
    steps: [
      ...
      "いちばん大きいのは「I am」で2（語数）。",
      "いちばん小さいのは「I'm」で1。差は1。",
      "大きい順にならべると I am → do not → ice cream → I'm → don't → e-mail。棒の高さを見比べて確かめる。",
      "語数の数え方。短縮形（アポストロフィでつないだ形）とハイフンでつないだ語は1語、離して書く語は2語と数える。コンマやピリオドは語数に入れない",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext12_437: {
    ...
    steps: [
      ...
      "いちばん大きいのは「理由①」で、全体のおよそ30%。",
      "内わけは 意見 20%、理由① 30%、理由② 30%、まとめ 20%。",
  ...
  lf_eigoext12_443: {
    ...
    steps: [
      "円全体を100%として、それぞれが全体のどれくらいかを表した円グラフ。",
      "いちばん大きいのは「三単現のs」で、全体のおよそ30%。",
      "内わけは 三単現のs 30%、冠詞 25%、複数形のs 20%、時制 15% …。",
      "割合は「部分 ÷ 全体 × 100」で求める。全部たすと100%になることを確かめる。",
      "英作文の答案でめだつ誤りのおおよその割合（目安）。半分以上が三単現のsと冠詞であり、この二つを見直すだけで失点は大きく減る。内容を考え直すより先に、まずこの順で確かめる",
    ],
    buildSteps: 4,
  ...
  lf_eigoext12_445: {
    ...
    steps: [
      "数直線は、右へ行くほど数が大きくなる。目もりの1つ分がいくつかを最初に確かめる。",
      "この数直線は0から10まで、1きざみで目もりがふってある。",
      "色のついた範囲は 3以上 かつ 9以下。●はその数をふくみ、○はふくまない。",
      "印がついているのは 過去の一点（last year）、現在（now）。位置を目もりで確かめる。",
      "現在完了は「過去のある時点から今まで」という幅を表す。だから yesterday や last year のような過去の一点だけを指す語といっしょには使えない。for（幅の長さ）と since（幅の始まり）はこの幅を説明する語",
    ],
    buildSteps: 4,
  },
  ...
  lf_eigoext12_450: {
    ...
    steps: [
      "よこじくは「設問の種類」、たてじくは「分」を表している。まず何と何を比べた図かをつかむ。",
      "いちばん大きいのは「読解」で20（分）。",
      "いちばん小さいのは「語彙・文法」で5。差は15。",
      "大きい順にならべると 読解 → リスニング → 英作文 → 語彙・文法 → 見直し。棒の高さを見比べて確かめる。",
      "試験時間50分の配分の目安。リスニングは放送の時間で決まってしまうので、自分で調整できるのは読解と英作文。英作文に10分、見直しに5分を必ず残す組み立てにする",
    ],
    buildSteps: 4,
  },
